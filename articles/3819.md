...
see_count: 4
comment_count: 0
stats_updated_at: "2026-07-04T11:32:05.279Z"
is_unavailable: 0
...
![](https://mmbiz.qpic.cn/sz_mmbiz_jpg/UicQ7HgWiaUb1P4hQ7icwjaUaJ0ooJmDa0DicBdSgrw5K25XiaKzoTHFoMve1RNaUMhUiariaa9eH3kA2icgWgViasZtGAg/640?wx_fmt=jpeg&from=appmsg)
...
来自UIC（伊利诺伊大学芝加哥分校） 和Adobe的研究团队提出一种模块化的编码器-解码器（Encoder-Decoder）架构**VGent，**其核心思想是：将高层的语义推理交给MLLM，将底层的像素预测交给目标检测器（detector），最终通过hidden state将这种解耦后的关系进行连接。

![](https://mmbiz.qpic.cn/sz_mmbiz_png/UicQ7HgWiaUb3421JoKBcNN2GZW2B2ct5KvJrJTiaqPtZDOZHjo5WvKnSLHWticnJuMspBHcMcIFV4bv8P3LnBzubQ/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/UicQ7HgWiaUb3421JoKBcNN2GZW2B2ct5Klf7alhPzVeaicbcac33nYnyHuuVQsWELXMccyM1mxZHeI6qsYicmfl1g/640?wx_fmt=png&from=appmsg)

_图一：VGent（蓝色）与现有先进的MLLM（Qwen2.5-VL，灰色）在多目标视觉定位任务上的对比。左图显示VGent的推理时间恒定且迅速，而 MLLM 随目标数量增加呈线性增长；右图显示VGent在F1分数上实现了显著提升，特别是在多目标场景下。_
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/UicQ7HgWiaUb3421JoKBcNN2GZW2B2ct5K0xB7arwVOffIdTQdpRGZzbkSXzpLic7h94Y5aIhlOxibmHfkXRozugUA/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/UicQ7HgWiaUb1P4hQ7icwjaUaJ0ooJmDa0DsqjYDckUJFmurYEanMlVAe0ICtPvCMIFBqvHZicZ04icVkxYhpEp900g/640?wx_fmt=png&from=appmsg)

_图三：QuadThinker所使用的prompt。_
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/UicQ7HgWiaUb3421JoKBcNN2GZW2B2ct5KEve4kkqzpLmEFFgLSnCI2MKFBzLBGSF7LoTIPFmLds0tCXzAE552Eg/640?wx_fmt=png&from=appmsg)

_图四：Mask-aware Label示意图。基于IoA的标签分配策略能召回被传统IoU忽略的细粒度部件。_
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/UicQ7HgWiaUb3421JoKBcNN2GZW2B2ct5Kibf4AX5qkKDzZqyJEjLkqz90Michs3lacOSLskhaLjv3zhMIuPSsMweg/640?wx_fmt=png&from=appmsg)

_图五：Global Target Recognition示意图。利用Learnable Queries注入全局目标数量信息，并聚合多个detector的结果以提升__召回率__。_
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/UicQ7HgWiaUb1P4hQ7icwjaUaJ0ooJmDa0DVWgKHRib8VL3picG7potTNr0pj0Vib067awq5XsceWRbh94sSxRgia1TVg/640?wx_fmt=png&from=appmsg)

_图六：在 Omnimodal Referring Expression Segmentation (ORES) 上的性能对比。ORES是多目标以及存在视觉参照（w/ < mask-ref >）的视觉定位基准。_
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/UicQ7HgWiaUb1P4hQ7icwjaUaJ0ooJmDa0DBb62SJatjNiczCN8NgdNLZyZ3lriall6TljwvnHZreQPsUArXlToNu0g/640?wx_fmt=png&from=appmsg)

_图七：在referring expression comprehension (REC) 上的性能对比。_
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/UicQ7HgWiaUb1P4hQ7icwjaUaJ0ooJmDa0DwBhOerunxIe4ofhibTJib9AHzOyR4mzusHvJ6YryoNia3tQ3vcldwnrWg/640?wx_fmt=png&from=appmsg)

_图八：VGent在不同挑战下的预测结果可视化。_
