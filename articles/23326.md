---
id: 23326
title: "时序存储：影响成本与性能的设计选择"
url: "https://mp.weixin.qq.com/s/5hVweujDDOhvIOK7O3axdQ"
publish_time: "2026-05-22 16:41:13"
account_name: "InfoQ"
read_count: 758
like_count: 5
share_count: 38
see_count: 3
comment_count: 0
stats_updated_at: "2026-07-09T03:11:18.217Z"
is_unavailable: 0
...
**![](https://mmbiz.qpic.cn/sz_mmbiz_png/hLLZnAbUwNgac1NNnUeFTQU8ERKDkvhWO5JnVJIAxCIaY2gHl9fKxf6OjaPkOvntOH8cIeGldGibnfSBqezaNH6uNCda9JqwyDuPZRKVnE0k/640?wx_fmt=png&from=appmsg)**

**表 1：维度和度量示例**

![](https://mmbiz.qpic.cn/sz_mmbiz_png/hLLZnAbUwNgdPk8tmzcW3UL4Mdia1AFB2yLsxvMHOwQibjPjF8dvpO8HwWNLpYic6Ntres2Ulsu7FtDke61m5gCRgDcY6m2GIZRVKWb6doDiaww/640?wx_fmt=png&from=appmsg)
...
这种划分很重要，因为它们在查询中的用途不同：维度用于过滤和分组（WHERE、GROUP BY device\_id）；指标用于计算（AVG(temperature\_c)、MAX(humidity\_pct)）。
...
规范化操作会将稳定的标识符移入 series\_dim 表中。每个测量值都会引用一个 series\_id，而不是重复使用标识符字符串。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/hLLZnAbUwNiagE5VZjNj1ZVXpibX4jJSZ6gYKMJcPtz3gBXhTqm5vb4tIbfDpv1QFkp8daQI5G7SEUMJSJ3ARmkaXTdO8TABnYjqNhSfBdUlk/640?wx_fmt=png&from=appmsg)

**表 2：扁平化模式和规范化模式的存储开销对比**

成本模型

效率差距取决于维度字节数与什么相乘。对于扁平化数据，TotalBytes ≈ N\_rows \* (timestamp + metric + dimensions)。对于规范化数据，TotalBytes ≈ N\_rows \* (timestamp + metric + series\_id) + N\_series \* dimensions。

这两种模式的总存储量均为 O(N\_rows)。区别在于每行的开销：在扁平化模式中，每行都携带完整的维度字符串。而在规范化模式中，每行仅携带一个空间占用较小的序列 ID，而维度字符串则每个序列存储一次。由于序列 ID 远小于完整的维度数据，而且在高频监控中 N\_series ≪ N\_rows，所以在行数相同的情况下，规范化模式存储的数据量要少得多。
...
**![](https://mmbiz.qpic.cn/sz_mmbiz_png/hLLZnAbUwNjiaIAPoLodQsA0xqA6u7ByCiaXqOSnibR69rpUcJtrOuFAnHU035z62Rr2pBQmib2aX2OUPj3AibcSiag5xfU4zV1YljpuDAHI5xsCA/640?wx_fmt=png&from=appmsg)  
**

**表 3：维度可重复示例**

这里，N\_rows = 3 且 N\_series = 1。规范化布局将维度信息仅在 series\_dim 中存储一次，并在每个数据点中复用 series\_id。

情况 B：事件唯一标识符（规范化效果减弱）

![](https://mmbiz.qpic.cn/sz_mmbiz_png/hLLZnAbUwNg5uGEJEiaZH1iau7Nq2rOY7kPcD0d4sL2NsLbzaJYWFUDnRsjcr7fic9sxPyW7vjXp43a5JxphhXyiaondmeUuW25yiblEnrW2c2vU/640?wx_fmt=png&from=appmsg)

**表 4：高基数维度示例**

如果 request\_id 是序列标识的一部分，N\_series 将趋近于 N\_rows。此时，规范化存储的去重效果将大大降低。从实际应用的角度来看，应将稳定的维度保留在序列标识中。此外，除非查询模式有此要求，否则应将 request\_id 等事件级 ID 排除在序列标识之外。
...
固定列的模式在标签演进之前都能正常工作。新标签的引入会导致表结构发生变化，需要进行数据回填以及索引更新。对于时序系统，一种实用的模式是维护一个以 series\_id 为键的 series\_dim 注册表，并将序列属性存储在 jsonb 类型的 dimensions 字段中，如下所示：

    CREATE TABLE series_dim ( 

这种方法在标签发生变化时仍然能保持数据摄入的稳定性。这里的权衡在于，索引构建变成了策略决策，而非一次性 DDL 任务。但索引设计仍然需要考虑。针对维度的通用倒排索引（GIN）对于一般的包含关系和键的存在性过滤非常有用。InfluxDB 采用了类似的倒排索引理念，最初出现在 InfluxDB v1 的元数据索引中，随后在时序索引（TSI）中得到进一步发展。Prometheus 采用了类似的 TSDB 索引模式，在分块读取前将标签匹配器映射到序列 ID（参见 Prometheus 概念、Prometheus 存储 以及 GitHub 上的 Prometheus 项目）。此外，对于频繁查询的一小部分热门属性，B- 树表达式索引非常有用（如 dimensions->>‘region’）。
...
![](https://mmbiz.qpic.cn/mmbiz_png/hLLZnAbUwNiapvLgYWsSXCW36wZ9f5R775EelGDxqjGcG8kp75bMeDw2mLg0n2OwEEmtaAcJKT9icpEpR7YhiayHSPA98lGqJ4cNfTEeYLCMiaU/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/hLLZnAbUwNgjXsvLmuaBCkuzeGoINvibb1KWQHofOPhB4B0XAHRKTf6OlY6Vz5mPQ5btZUxGY4ORFJfpWEaaWa3NP6Dxj48M1iaA0j56aMcPE/640?wx_fmt=png&from=appmsg)

**表 6：窄模式：对于每个时间戳，一个指标一行，序列标识与时间戳在每行中重复出现**

宽模式一行存储多个指标列：

**![](https://mmbiz.qpic.cn/sz_mmbiz_png/hLLZnAbUwNjnCPvvhUhFy0lfZIYVU56TtryL2HqkbL4uBP3QhcQZs6OJrxgyfj7n7IyCdWy5UpYXZgsqXzvYq9eTEk6YU9tIoLCtqicpACpQ/640?wx_fmt=png&from=appmsg)**

**表 7：宽模式：每行对应一个时间戳，所有指标均作为独立的列存储**

权衡：

*   窄模式：每行指标数据都包含 (series\_id, ts)。
    
*   宽模式： (series\_id, ts) 仅针对该时间戳存储一次。
    
*   如果需要查询对应同一时间戳的多个指标，窄模式通常需要在 (series\_id, ts) 上进行转置或自连接。
...
包含 WHERE ts BETWEEN ... 子句的查询会自动仅匹配相关分区。可以通过执行 DROP TABLE readings\_2026\_01\_15 来删除旧数据，而无需进行行级清理。
...
通常，“空间”维度是由序列标识衍生而来的：例如序列标识（series\_id）的哈希值，或是基于区域或设备类型等的自然分组方式。

![](https://mmbiz.qpic.cn/sz_mmbiz_png/hLLZnAbUwNgukDorbkfiaobPUpIrGuqesfJTwmHibuKvkhw3vJU5hQ9pFtud9ZbKEgsj7z2OoJjicXwic7KvYBI104w6WuKIMj1mqhz147OF5Ss/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/hLLZnAbUwNhxZb4YQtU566cy23WtH1VVYzicR1fCXJOSfROu8hrCdF2Ry3sfABL4sTnPTD9KSVBuVX3Bq1PWopJRWRc8j1uUibsNlUvEzTdJY/640?wx_fmt=png&from=appmsg)
