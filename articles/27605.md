---
id: 27605
title: "Arm CEO：CPU需求已「爆表」！"
url: "https://mp.weixin.qq.com/s/QS3xSOwlmq2sGgPCqKmTIg"
...
see_count: 10
comment_count: 1
stats_updated_at: "2026-07-09T02:52:11.322Z"
is_unavailable: 0
...
似乎，相较 GPU，AI 时代的 CPU 只是个干杂活的「配角」。
...
近日，科技记者 Tae Kim 采访了 Arm CEO Rene Haas（雷内・哈斯），后者直言，当前市场对高级 AI CPU 的需求正处于「爆表」（Off the Charts）状态……

  

![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqFPdqiaba3FjneVRicmmNYCCEpVnMUUyJsaZS2D5ccVmciaA3IhXcBHiczm9IWs0lh2lNeD2s6DHxKibJuMSv4b1uC0RWK0vhdcgcAI/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqFktZCcrlZ7freANDRDkz2rKTtvUFlYC3N87DSw9ARv7dEmWJKiauRibpI9WDQLUG7yINk6vvJKtAHNTIH4NiaW3Lo0twWK4icJtGA/640?wx_fmt=png&from=appmsg)

Arm CEO Rene Haas

  

资料显示，自从今年 3 月下旬，Arm 在旧金山的一场产品活动上发布全新的 AGI CPU 以来，公司股价已经上涨超过一倍。当时，Rene Haas 就曾预测，随着 Agent 时代的到来，每 1 吉瓦数据中心容量将需要 1.2 亿个 CPU 核心，而去年这一数字是 3000 万个。
...
Rene Haas 坦言，大约一年半前，当人们开始构建基于 Arm 的 SoC 时，他们就意识到这一趋势：AI 时代的 CPU 需求将「爆表」。
...
经过一番研究后，Arm 发现，是 Agent。在这些 Agent 工作流中，核心越多越好，因为可以直接启动一个 Agent 去运行一个虚拟任务。

  

紧接着，Arm 又去观察其他相关趋势，然后意识到：CPU 需求真的会因为 Agent 的编排、调度和管理而「起飞」。因为这些负载无法简单交给 GPU 完成，CPU 才是支撑 Agent 系统持续运行的关键底座。
...
Rene Haas 举了 Vera Rubin 为例来阐述原因。NVIDIA 的 Vera CPU 机架是在一个液冷机架中集成 256 颗 Vera CPU。一个数据中心中，可能这一区域部分全部采用液冷，而在另一个区域部分使用的可能全是风冷机架，那么那里有存储，有网络，也就可能正好放着 Arm AGI CPU，它们都是 OCP 标准机架。
...
总的来说，Rene Haas 的这次采访透露了当前 AI 火热趋势下的另一个切面：AI 的终局不仅是 GPU 的比拼，更是能效比和 CPU 调度能力的博弈。 
...
这无疑是在说明，一个新的行业共识正在形成：AI 基础设施不能再只围绕 GPU 来理解。模型之外的系统执行、数据调度、工具调用、并发编排，会成为下一阶段 AI 工厂效率的关键变量。  
...
或许，AI 基础设施的下一轮争夺，已经从「谁有更多 GPU」扩展到「谁能支撑更多 Agent」。
