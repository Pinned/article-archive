---
id: 26609
title: "不安全指令，一拒了之？TRIAD用三路决策：修复AI智能体的危险计划"
url: "https://mp.weixin.qq.com/s/wjom-lT9RUH4Pq1Mpp-pQA"
...
see_count: 16
comment_count: 0
stats_updated_at: "2026-07-09T02:22:59.273Z"
is_unavailable: 0
...
![](https://mmbiz.qpic.cn/mmbiz_png/Rvq8Ow69CYWvxhaB8N81UicDTbcan8nZy1JB5hsMqVRI7459uibAVIF3DdDbIWzalKZqiaricBkP7RDjeiaAjuic4Zia0FMkTicrLPKCySogq2cDhg0/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/Rvq8Ow69CYW1NuaAff2730TcefpAYiaqKvMx5rAJmDhzb1sfIT0PfaWOe0DgD9K3jszjTBiaGuoOTXXwMpEOibYfRKMLe7Dv3tqFK55libI6LL0/640?wx_fmt=png&from=appmsg)
...
为此，本文作者团队提出了**TRIAD（Tripartite Response for Iterative Agent Guardrailing）**，一个面向 LLM Agent 的反馈驱动护栏框架。TRIAD将传统的二元护栏决策扩展为三类：**Proceed**、**Update**和**Refuse**。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_jpg/Rvq8Ow69CYUCG0bhkhvfxZUx0qZxW9pZmPIficic1F9UJbcb1cFicr5b9vUtVQJlH3SPpWpyrAqo6duyB8QdN0kgXmDh4hHdjibmt5HAIkk7Z6c/640?wx_fmt=jpeg)

_图 1：TRIAD流程与baseline对比。 在Agent执行工具前，Tri-Guard 会检查其行动计划（action plan），并给出Proceed、Update或Refuse三类决策；对于被提示注入污染但仍可修复的任务，TRIAD 将自然语言反馈写回上下文，引导 Agent 修改计划并回到原始目标。_
...
其中，**Proceed**表示当前计划安全且与用户目标一致，Agent可以继续执行工具；**Refuse**表示用户请求本身有害，或当前任务无法通过修改计划安全完成，Agent应直接拒绝；

**Update**则用于处理最关键的中间情况：当前计划受到了提示注入或不可信内容影响，但用户原始目标仍然合理。
...
![](https://mmbiz.qpic.cn/mmbiz_jpg/Rvq8Ow69CYU0zibtdak0bw0IOG172ZNu0lcxYQicWEH6cWQImta7JIPjMXCX0uICsK7UNcZuSy4esfWsL6BLbUZicfcJUcDXd8hu4EdRHjdBv0/640?wx_fmt=jpeg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/Rvq8Ow69CYWUKJelqWCbvNJ29dicoyqKpn8BN4nv1tc3aYrOibyD6OLXAnamicVItyaYNmjz19dqyRMZwJxLostenMoBLQXcJA3Mpc2P5QW8IM/640?wx_fmt=png&from=appmsg)

_表1：TRIAD在四类Agent上的实验结果。实验覆盖ASB-DPI、ASB-IPI和AgentHarm，比较无防护 ReAct、ToolSafe、TRIAD + TS-Guard和TRIAD + Tri-Guard。_
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/Rvq8Ow69CYUwAzGyYuZUUjM91jlFibiaBQyTlfBIJTODicokibSvutwUcCYp2XoAwfTibYjvmXAIEKMsp4TUhvEwwuSpsiaia7smvYRd7ulhnCFpUM/640?wx_fmt=png&from=appmsg)

_表2：在同一 TRIAD 框架下替换不同 护栏(guardrail)模型的结果。实验基于 Qwen3-32B，比较现有护栏(guardrail)、Qwen3.5-9B base model 和经过训练的 Tri-Guard。_
...
![](https://mmbiz.qpic.cn/mmbiz_png/Rvq8Ow69CYXJ5bpxxFTicCvtnzkUrzscaDMrPTrX6xialNMdfQFtIOY1AZibZttSUTSCVpP53EEXM3nJGicicxWTU6FTQiaUsAHdRSXTzpzPDzrjM/640?wx_fmt=png&from=appmsg)

_表3：_Tri-Guard 与训练前 Qwen3.5-9B base model 的平均性能对比。结果为四个 Agent 上的平均值。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/Rvq8Ow69CYUhpCX7asZje67mIgZun4hkVx3EwlJwr9VLuKpDH9zgzRH8MGQzY3ddavlialsnaZQn1f2hpTWtkGlWpaicMHDTFqc1xY37Zw4ibA/640?wx_fmt=png&from=appmsg)

_图3：训练前后 guardrail 决策分布变化。相比 Qwen3.5-9B base model，Tri-Guard 更倾向于将 PIA 污染的行动计划路由到 Update，而不是直接 Refuse。_
