---
id: 744
title: "3D-AIGC 存储架构演进：从 NFS、Gluster 到 JuiceFS"
url: "https://mp.weixin.qq.com/s/GiVlF_coADbFK4WKwBKIOw"
...
see_count: 1
comment_count: 0
stats_updated_at: "2026-07-04T09:00:22.225Z"
is_unavailable: 0
snapshot_reason: "网络同步补齐"
topics: "[\"存储架构\",\"3D-AIGC\",\"演进对比\"]"
entities: "[\"NFS\",\"Gluster\",\"JuiceFS\"]"
---
...
区别于二维模型，三维模型单个模型可达几 GB，尤其是点云数据等复杂模型。当数据量达到 PB 级别时，管理与存储成为巨大的挑战。经过尝试 NFS、GlusterFS 等方案后，我们最终选择了 JuiceFS，成功搭建了一个统一的存储平台，为多个场景服务，并支持跨平台访问，包括 Windows 和 Linux 系统。该平台目前已管理上亿文件，数据处理速度提升了 200%~250%，还实现了高效的存储扩容，同时运维管理得到了极大简化，使得团队能够更专注于核心任务的推进。

1 3D-AIGC 存储需求

我们的研究主要集中在感知和生成两个方向。在三维领域，任务的复杂性与图像和文本处理有本质区别，这对我们的 AI 模型、算法以及基础设施建设都提出了更高的要求。

我们通过一个 3D 数据处理流程，来展示三维数据处理的复杂性。下图左侧是一个三维模型，包含左上角纹理和右下角几何信息。首先，我们生成渲染图像。每个模型还附带文本标签，描述其内容、几何特征和纹理特征，这些标签与每个模型紧密相关。此外，我们还处理几何数据，如采样点以及从数据预处理过程中得到的必要数值，如 SDF 等。需要注意的是，三维模型的文件格式非常多样，图片格式也各不相同。

![](https://mmbiz.qpic.cn/mmbiz_png/F446RY3QsZmOYycicLD0NmWlAVoel2HekiaeJRmcEgiabOJnarn3x8ia3wlFNgOHZvZOl7cDX27fz4Uoib06IoaBgSQ/640?wx_fmt=png&from=appmsg&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=0)
...
![](https://mmbiz.qpic.cn/mmbiz_png/F446RY3QsZmOYycicLD0NmWlAVoel2Hekia7Sy3hrXZzqnMWCb0p1EfZCSRQhvNTXWHgcRMmRg1NZHToWnKftuRg/640?wx_fmt=png&from=appmsg&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=1)
...
2 存储方案探索：从 NFS、Gluster、CephFS 到 JuiceFS
...
![](https://mmbiz.qpic.cn/mmbiz_png/F446RY3QsZmOYycicLD0NmWlAVoel2Hekx35Ww7ErZVUu07dcqPUiaTibYuqmDDwOHXDHQrdD8CTyrItHbEv1ggyQ/640?wx_fmt=png&from=appmsg&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=2)

基于 NFS 的存储架构：扩容困难、运维复杂
...
![](https://mmbiz.qpic.cn/mmbiz_png/F446RY3QsZmOYycicLD0NmWlAVoel2HekRhG2jPSibNyDO1cDTZaiacQNL3h4vPl835UibhSAKN4nibhgQV0CC9kfKQ/640?wx_fmt=png&from=appmsg&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=3)
...
![](https://mmbiz.qpic.cn/mmbiz_png/F446RY3QsZmOYycicLD0NmWlAVoel2HekKp413Y3OkhfcrCLgOezFRV7MXnPKefmHibuX6icuXr5gV0dt6XKVQMNQ/640?wx_fmt=png&from=appmsg&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=4)

CephFS vs JuiceFS （来源：JuiceFS 文档）

3 基于 JuiceFS 的存储平台实践
...
![](https://mmbiz.qpic.cn/mmbiz_png/F446RY3QsZnu8ibfmTibYKqRQibHTAa96jibnpaWBc15Z6WXXctEE2w8JdBLXEaiabUI1rQn7lZV5iaAAmGoicK7nPTcA/640?wx_fmt=png&from=appmsg&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=5)
...
![](https://mmbiz.qpic.cn/mmbiz_png/F446RY3QsZmOYycicLD0NmWlAVoel2HekUoPCIU5gs5riav1W5kpDyoUrkkrkTIVwnnMCe40wCUkoV3ducWkuOmg/640?wx_fmt=png&from=appmsg&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=6)
...
![](https://mmbiz.qpic.cn/mmbiz_png/F446RY3QsZmOYycicLD0NmWlAVoel2HekNFEtIRxTXrDicFNW5y1icNPRNLOsDEWRgYprmM4TLFedy6BvFTQXZfnA/640?wx_fmt=png&from=appmsg&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=7)

存储架构： JuiceFS + TiKV + SeaweedFS
...
![](https://mmbiz.qpic.cn/mmbiz_png/F446RY3QsZmOYycicLD0NmWlAVoel2HekZwvxt4kYrNG2xGdgiciaH3FXZ9DbQV978K7mB7KPTDsVKiamzKiaxUAO4w/640?wx_fmt=png&from=appmsg&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=8)
...
![](https://mmbiz.qpic.cn/mmbiz_png/F446RY3QsZmOYycicLD0NmWlAVoel2HekIkfOYKWaVzcFaU9ALRDb7EYbe7tDgSeDh8icC1IdNCzNxkDHY1XTVibw/640?wx_fmt=png&from=appmsg&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=9)
...
李威宇，光影焕像算法技术负责人，负责三维物体及场景生成算法开发以及数据平台的构建。
