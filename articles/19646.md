...
url: "https://mp.weixin.qq.com/s/QkASULVptCbq4v0oz3zrbA"
publish_time: "2026-05-11 08:07:07"
account_name: "量子位"
read_count: 19797
like_count: 219
share_count: 1260
see_count: 129
comment_count: 9
stats_updated_at: "2026-07-08T10:03:58.318Z"
is_unavailable: 0
...
![](https://mmbiz.qpic.cn/mmbiz_png/A6fTew8FFGGaED8yazuFIMibAjHqyn5GaZ5icFLwsjs3ibWyGzBnmZtHXWVjXmdyCjObIOmXdeGECKm9vf557ib56akrI4icZnnXu4HtjQFzXH70/640?wx_fmt=png&from=appmsg)

它的表现有多夸张？

*   **仿真满分级别**：在LIBERO benchmark上，仅靠1条轨迹预热，平均成功率就冲到了**99.9%**；  
    
*   真机性能起飞：在真实抓取、旋转等复杂任务中，比目前最强的SOTA模型π0.5还要高出**22.5%**。  
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/A6fTew8FFGFaK79Zt5sa0n5sDcamNvLib5UL7xmbPOXP0n1t2TjaTRlNTXBngWmGI2QRNYLtURyd647oteiaTEcWunRcMX2m9OiaROyIuzRBLM/640?wx_fmt=png&from=appmsg)
...
*   LaST-R1在模型推理中引入**latent CoT**：生成动作前，先在latent space中建模当前场景、物体关系和未来物理动态。  
...
*   LaST-R1提出**LAPO**，把环境奖励同时作用于**latent reasoning**和**action generation**：成功轨迹不仅强化正确动作，也强化动作之前的“好推理”；失败轨迹不只修正动作结果，也反向调整内部物理推理空间。  
...
*   LaST-R1引入**adaptive latent CoT**：简单状态下，模型可以快速结束推理并执行；拉拉链、擦花瓶、拧瓶盖等复杂接触式操作，则分配更长reasoning horizon。  
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/A6fTew8FFGF3NMUasVgUcFCW7U57dd0cPVA399ic89VwEW90uw5kp037GgbzSIya7FRibygkaX5OkvjY6PITLVdVW4txQMd4Z6GFM5TcvGzGw/640?wx_fmt=png&from=appmsg)
...
其中，表示rollout时旧策略生成的latent sequence，表示当前策略重新生成的latent sequence，控制latent分布宽度。

直观来说，如果某条轨迹成功，LaST-R1不仅会强化对应动作，也会强化动作之前产生的“好推理”。

随后，LAPO将latent和action放进统一的clipped objective中：

其中，表示同时优化latent reasoning和action generation，是advantage estimate，用于限制策略更新幅度。
...
为了优化这个结束标识符token的自适应生成，训练目标需要进一步加上L\_end。  
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/A6fTew8FFGFC2DoFcoA7o7RIh9Sf10iaib5JkUNjtwicsWvBH2dquLUxicGlPZ1wLiaiaUzNzxL8f2v36UIQcUJxCicESvhfBMejNqjIfmvet8Yqrg/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/A6fTew8FFGFCVnGl6hUkDVPSZgvX23tS4ibzRQfZqGzicTMJs07jncCRh3XEnOtHo4ia9rSPMiaLJciamlPDzYjUUL4lMbp1pdwwXQyFn3lGibn0U/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/A6fTew8FFGEX5zIsLFSVcWSNTbEdO0B9gTYqeibjQlKO3WPo12BL6cjYyTAicw0W2n4q10srzia9uhRkOTXzuhyb8iasyvtfaEhicyPPdpp8ZPmY/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/A6fTew8FFGHmiaskteEZoqu5BIk17SVQzhfe4OTJPicTOxKoM1wmxcMMkJoyug5CbfZ3VOC0Tv5Xk9iaqyL35LOwJFJz0VibsY7C4yAK7W9069Y/640?wx_fmt=png&from=appmsg)
...
_论文链接: https://arxiv.org/abs/2604.28192  
项目主页: https://siriyep.github.io/last-r1/  
代码链接：https://github.com/CHEN-H01/LaST-R1_
