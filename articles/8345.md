---
id: 8345
title: "源码被开源，Claude Code之父回应了：纯内部开发者手滑"
url: "https://mp.weixin.qq.com/s/jqh1zNM69BSAp_AsL6l9Kw"
...
see_count: 697
comment_count: 62
stats_updated_at: "2026-07-04T12:55:21.969Z"
is_unavailable: 0
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqH6KIJgHcB8fFNRKzQiadT9Vk31McMJmFn8T6laV4E7PS1S7sXlUJ75JZ8zEXVJMhXCnPSsICd9Wgia4fJC0YOjJeNOLnzJPCHWY/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqGPx8mIDMEOVC9TSPYSpYjhfagok0JBRrn6LTCRic4HSC8ccDFVrku6JmhXRwBIDuZQpvg0RicbURoPJPtkQHkLcicxw38jBPFyyI/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqEZxhNyVnAulL0zIUvoy7gk7qibAqvtgKf4TxjLKvgooyicx5BUw58XMYXSWC2G6hicJp1fRsbF6MJKwV9gc1GicHZWsMqvo4GVicEA/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqGbvf2NkhCrDojYOoIxeX4lmU0jqKz9Ny5IarUEVA9wo8qsYdTRo9VXURdlRNbS0I11a1BDuGYicUywpyP26moGCXgTUxR6Woes/640?wx_fmt=png&from=appmsg)
...
隐藏的 Simple 模式。当设置环境变量 CLAUDE\_CODE\_SIMPLE=1 时，整个复杂的 system prompt 会被压缩为一行：「You are Claude Code, Anthropic's official CLI for Claude」，并附带当前工作目录与日期。
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqET76kJYYFYHpQyFpXw05hVx8cOibaVzFaF0ib9379RCH7d05chP9sswY9vHT8Huw1qe8deVlibVsLVq6dicDTXrd8UibscTiaThia5so/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqGjhaAic0ocD5gKzUZkqhpYN1MnTql0xJnvCr9p7LgF7iaKok4kXFmJX5c9yvFJYQsrpiaoicGvFekc8BiaCUCicHarvSs18IwNqZKpY/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqERWHhicibNKBZkNQjeVlxw5hTJLnmLW1Wibp6F09TA6xGZHZBhoUEa1XvXmibfKARE6Tw0Rf5Hb3TxVds7eIYwq0zLP1AjLbONrLM/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqGrsyxTtdGqgBpyumgEqw7XHV0Dp5oVHicNeQibhfEumcwEh3Oo0h7YmBLsJOvCxRibkHcbhXlAZnhjkRJPs1XIhxWicZrbEPice3SE/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqExqzD97buicr6WAFwwtgV79lk6YrNuggeJyVhVdNiapJ0nfzjZ4SyXCYMDLthPJ70uZH5mUA73KjENiaicEQMoXicqLMgNmwibeN14E/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqEzYgxma35Poia39ts3fpg3l7qnDa4y7KSt6Tic8R9cDicASiccePSfF8w0dryFz0YlSZhwABTmuh1UQs821NRGYMcv0LE0v5SDZXw/640?wx_fmt=png&from=appmsg)
...
在 services/api/claude.ts 中，有一项通过 feature flag 控制的机制：在 API 请求体中加入anti\_distillation: \['fake\_tools'\]。
...
system prompt 被 SYSTEM\_PROMPT\_DYNAMIC\_BOUNDARY 分为两部分：上半部分是静态且可缓存的内容，下半部分是动态且随会话变化的内容。MCP 服务器相关指令被从 system prompt 中移除，改为通过 message 的增量附加（delta attachments）传递，因为如果放在 system prompt 中，每次有服务器连接都会导致缓存失效。

  

子 Agent 会从父 Agent 继承 CacheSafeParams（一个包含所有影响缓存键参数的结构体）。源码中有一条注释警告：在 fork 出来的 Agent 上设置 maxOutputTokens，可能会意外限制 budget\_tokens，并在旧模型上破坏缓存兼容性。
...
utils/undercover.ts 中实现了一种卧底模式。当 Anthropic 员工（USER\_TYPE === 'ant'）在非内部仓库中工作时，该模式会自动开启，而且默认始终为开启状态。只有当 git 远程仓库明确匹配内部白名单时，才会关闭。
...
最终的解决方案是：设置 MAX\_CONSECUTIVE\_AUTOCOMPACT\_FAILURES = 3。当连续三次压缩失败后，系统将停止继续尝试。
...
*   自动压缩触发阈值：context\_window - max\_output\_tokens - 13,000 buffer
    
*   强制压缩（阻塞用户）阈值：context\_window - max\_output\_tokens - 3,000 buffer
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqFPic60MGs7Gmw9sKb86ce7uOsIoPDUNaibYl2QJnUoyzpLiadU0QFjcF7wFaYUF0AloY6aRVo8UgszHH6TteH3UwwlKlPUGuUXyM/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqHnDS422MeFx7Z5ic5KwvegeAAUGM25dCv2bsIUhVH3dIHs2NNo6M4DpicXbFEOOxeK8e1OVCIS6XdgKWs6UU5zEic1zXrwj3nicSo/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqHOcBSxEagtOT2lx1xLReiaicEXLIJxIeJada2phEbyqK1kKkiaZYxSaLTvkfd6Is9moToerujpTywwHyIu2JdChgnmKJCLnZzicFo/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqHnYrGUpTibzzukFicgLfEvNBqzNz6BU0k860iccuAdqW6dV68iaghZsNrmibQ3Kll6ibGq8kFHmjos203QNUETvLicCEsE9Byhp9ZSUY/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqF4nvPH6FFgN6119FH7yPPkqvOJJ6iasDAh3ngB6VVwfEMsnVibKibq84KDIUoYs6RibbISPqxaVkHse2ib1QjNJ5rsmvk0u7DHmPFg/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqHTJY03Znp42ibzruDu4t3FjBxjFjVT89eXVrZFWb8SxrMVQxdRw0ANkYDKZIlSSk228B07nrr2RkPlhwibSYWsoLkSfBVkBbb3E/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqH8CzIkA2ASEu7seZb8zVFc1eXJ81xOVzdDyH9Q7OVOibgjACFY22SEh8A9mT5F2DTWeqtsLSTH4xIZmicuR4uwcvCvR37yckqpM/640?wx_fmt=png&from=appmsg)

https://x.com/rohan\_2502/status/2038927786228998194
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqFm8M0A5K051gMESJtBWHQVRMibXPPbiaDYpWPPu8mOcxOOUQVaoMmOdlg8oueeYudfVzFZ427qHvcBFcanfOMxAgtYrJVYrxibgY/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqFHPmCBCBzsHmMe2cd1NZtQj3CNqGTGNqGxySwaADmtOXXZUrfsj4eibnOMnOicWr30KLWD3M5NYg4ZWP2UE3H5qw7bGQREribOA4/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqHibWcudPoSbbQElU5j3mXZL2gvib7sZvd7J9uTDKxhEqsZ4qR2PnSdSuNPWbvhkicM98rWliaRhXCW9LZlOYvnyxVwichIvWyAgSMI/640?wx_fmt=png&from=appmsg)

  

![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqFgyaNQTibljQEcfZkKqCWC2w4rmzOqjR1aGGxPmAPtyEvGrdXx2dFQ9noZMVGuUJJ2HKAOp6IFeK8uOkM4UHWj0xDgyYtibF0Zs/640?wx_fmt=png&from=appmsg)

  

![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqFAI5RFKd5JicC0hyn4lf0DVCQ7XROkIN1GOvAibDXx49J3LpichmHhV1Yib82cjZfKdh5CIRTV14Ku41iczfpSLBgEM50n8g0lf5T4/640?wx_fmt=png&from=appmsg)
