...
see_count: 8
comment_count: 0
stats_updated_at: "2026-07-09T02:30:35.966Z"
is_unavailable: 0
snapshot_reason: "网络同步补齐"
topics: "[\"Next Token预测\",\"大模型\",\"算法改进\"]"
entities: "[\"ICLR\"]"
---
...
![](https://mmbiz.qpic.cn/mmbiz_png/Rvq8Ow69CYXE4hqpAOV5HHFE4eboicbEUqdegT3RsrKYmkB7h5IKcYyknQoLMPeQA3cl9goqIEYIficY0PlJZzrgUrQM5LC6aSgpO131V6GibI/640?wx_fmt=png&from=appmsg)
...
##### **【新智元导读】大模型常因只关注当前预测而显得短视。Next-ToBE通过调整训练目标，让模型在每一步预测时兼顾未来token分布，从而提升整体推理能力。**
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/Rvq8Ow69CYUacyTPWCOicGSD4Nr1O4azx8ENzRIZd0Mb7PaZ1aIqibQ3qRPhlP4Zic05VrGyKA4Q9PoHmA4M2jWibw4ZOHs4Iic4EVBYT7rcia44w/640?wx_fmt=png&from=appmsg)

**论文链接：**https://openreview.net/pdf?id=T8IJojfaOh

![](https://mmbiz.qpic.cn/mmbiz_png/Rvq8Ow69CYUVr9GjNpKMguFrS315HQdsaPcQ2h3F46LEgyXYM2CuogjfHiaex7EibvcCich1ZrzZZLUPWhC1hP3JFalGyTZzpcNdXhIpE3blz0/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/Rvq8Ow69CYWaicFFJ8ibWoHfIKicSL27eicIiaFicwGZdfml4CptEXgfX7jd9YrXFtZQiaLMZ5UibjZRr7r3IgQSs3vfIZRicqnAAM5iafSByPEa2PG5o/640?wx_fmt=png&from=appmsg)

图 2：左）当前一步的输出分布也已经覆盖了相当比例的未来 token；右 ）未来 token 在当前预测中的排名越高，后续被正确生成的概率越大。

实验结果（图 2）揭示了两个关键现象：

1.  当前一步的预测分布中，确实已经覆盖了相当一部分未来 token；
    
2.  一个未来 token 在当前分布中排名越靠前，它在后续生成中被正确输出的概率也越高。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/Rvq8Ow69CYXDgqkkhZZoRDbWIJDPObc1glUeKiaQHTRjicPZ44XKkHm5jfQe1gxwkFL6TIpCPuTyS8lVkdrV1bub6y8FSbC1zPSXUUlwOUWOc/640?wx_fmt=png&from=appmsg)
...
1.  **当前 token 仍然是主要目标。**Next-ToBE 并没有放弃 NTP，而是将「预测下一个 token」作为损失函数的主项保留下来；在此基础上，引入未来窗口内的个 token 作为辅助监督，并通过系数  控制两者权重。这样既保证了局部生成的稳定性，也让模型对更远处的内容保持敏感。
    
2.  **未来 token 进行时空结构化加权。**未来k-1个token 的权重由两类信号共同决定：一是模型当前对该token的预测概率（即「模型本来就觉得它可能出现」![](https://mmbiz.qpic.cn/mmbiz_png/Rvq8Ow69CYVStQzmCeXUibZfx5f6TicibrmLhzZQ1Q42Iq0dEbiaCFpKVvLKjr0lbumVCcIVw8rNHaLJm9MfTCL6DvB4T6FD19fDia4VddsZlEto/640?wx_fmt=png&from=appmsg)），保留预训练阶段形成的先验；二是未来 token 与当前token之间的时序和语义关联—越接近当前、语义关联越强的 token，权重越高（通过交替随机游走建模![](https://mmbiz.qpic.cn/sz_mmbiz_png/Rvq8Ow69CYVMXkHGJtjPE2zGkJgGeQliad2kg1ERPkUicKHNA63YLK5otkkst2xzHBLV2KtnFCZzI9z0icAJsyEZ9h3T08OH9aIGc3h8sxfQTY/640?wx_fmt=png&from=appmsg)）；最后融合成![](https://mmbiz.qpic.cn/sz_mmbiz_png/Rvq8Ow69CYVQX8CaVL9ObvDCic9UknwJK9SGXUC9yoV3hkgQpjBk5WHBhWTBQib33fgeD8RibnlcgvYgdtBV4oKAjq7tl0yMkibiceHKqnqlJ3hw/640?wx_fmt=png&from=appmsg)。
    

3.  **目标分布的归一化。**对于未来k-1个 token，其权重![](https://mmbiz.qpic.cn/mmbiz_png/Rvq8Ow69CYUrcgNgP5ZYQuVDLG593ibA4JjrcoHJuNjwnk3vmtJdEgMFupOU0vG1V1iaUwgx6TGcaCniaLicZB2MRiclUtd9aBRNQS1C8zfEvgB8/640?wx_fmt=png&from=appmsg)经归一化后构成目标分布；同时，对模型在这些 token 上的预测概率进行相应归一化，并最小化两者之间的分布差异（如 KL 散度或交叉熵）。
...
![](https://mmbiz.qpic.cn/mmbiz_png/Rvq8Ow69CYXEUnHsHEfGxaJ80g1OGwhQ0aEChz6bYyMmjeAsC938ib1xefzwVHy99xfxxVnGib2vJjue6OlDpOeODK65XgTErPGHM3suIIQfY/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/Rvq8Ow69CYX3ru9QqVdUkyxN2GfM8xpqOePibIn4sY47Lr26HjekGLKIRiaPXuRmdjjTvYO8ragic9UJcauFDwTbMxpAZSw5a3qhs6iahmLqHg4/640?wx_fmt=png&from=appmsg)

![](https://mmbiz.qpic.cn/sz_mmbiz_png/Rvq8Ow69CYW4ia5JTgAoKDaicloCPSyFGs6xibKqE4GgjLX510Yymw6seRYHBL7vSgR0Z94w7H6ohJy0IAlfiaibV2a6YFrTDPQKha6fXqCh58tg/640?wx_fmt=png&from=appmsg)
...
Next-ToBE 在更大的尺度上实现了对**确定性分配**的系统性调节。随着超参λ增大，模型不再把全部注意力压在「下一个 token」上，而是逐渐将一小部分概率质量分配到更远的 token 上。这一变化直接带来的结果是：下一 token 的预测置信度持续下降，如图5（左）所示。

![](https://mmbiz.qpic.cn/sz_mmbiz_png/Rvq8Ow69CYUgoqF219EYkuCWHayiaicqIZDaRKoBkRWwyle9QOgic5jz4eNNibReAO4jicnrMexTy3Afpl2iaNvwVYJIzp19RJ5IMFI7p9G9tnTbo/640?wx_fmt=png&from=appmsg)
