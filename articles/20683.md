...
see_count: 99
comment_count: 5
stats_updated_at: "2026-07-08T10:03:12.852Z"
is_unavailable: 0
...
如果你最近在**Kimi K2.6的Agent模式**里敲下这句话，5分钟后，你拿到的不再是一堆需要自己调试的Python代码，也不是一个只能看的静态Demo。

而是一个真实可访问的URL。

前端、后端、独立数据库、用户账号体系……全套齐备。你可以直接把链接甩给朋友，他注册后存入的数据，会稳稳地停留在你这套系统的**独立数据库**里。

比起v0或Lovable这些AI建站工具，Kimi实际上接管了用户**从开发到托管、再到数据库运维**的全生命周期。

![](https://mmbiz.qpic.cn/sz_mmbiz_png/A6fTew8FFGGia4bqKtE29ggBG073icFhH5EdZr5rK0HdKTibVmbxGKVibgHxrBgr1Clu4icUqxCKnqdA802bAR0OCDDjlS6V8iagUqK9bhOibGUggM/640?wx_fmt=png&from=appmsg)
...
一旦运行起来，托管的基础设施成本_（web服务器、带宽、数据库）_相对算力成本要低得多，厂商的利润空间主要靠这一部分。
...
按传统云数据库的定价模型，一个最小实例大约每月十几到二十美元。乘以百万，账单天文数字。问题不是数据库贵，是**商业模型无法规模化**。

**第二条：数据库的schema是LLM现场生成的**

**（注：schema指数据库模式，是定义数据怎么存的逻辑结构。）**

过去二十年，schema设计是一个需要DBA_（数据库管理员）_、需要review、需要版本管理的慢决策流程。
...
所以，数据库必须同时扛住**“绝大多数近乎零、少数瞬间爆量”**的极端曲线，而且要做到**爆量租户不能拖垮其他所有租户**。

![](https://mmbiz.qpic.cn/mmbiz_jpg/A6fTew8FFGHoyKtaddX8CCHiatEKFiakGaCh60uAqwLPAHmMlbccX9ma0RcpOyiaFLXDOe3icZt7icx4phgOH2oYsVyK6MPjphSZUQlibD3jxly4M/640?wx_fmt=jpeg&from=appmsg)

这三条合在一起，在传统数据库的产品形态下，几乎是**做不出来的**：
...
![](https://mmbiz.qpic.cn/mmbiz_png/A6fTew8FFGFO0D7YzEZyewicqKj5Z3ZFoGnwmt629ibZmmsBwGiaBQTo6dZKian7IiamrFrcE9pFBwe9c3kMDIz9uOoc5SIfFos7BYmsWWJc46WM/640?wx_fmt=png&from=appmsg)

下面是TiDB Cloud的多租户：

![](https://mmbiz.qpic.cn/mmbiz_jpg/A6fTew8FFGFW5PDmwbOXicApZL4Zickp3uLxrFuJ90gibLDOWXyvoENo7pdwbl0bmKgHNcZSslQtjiamlib99SVeg6ic6BQAWgJ334v3tIAuic04Sw/640?wx_fmt=jpeg)

**决策二：统一技术栈——vector+SQL+JSON把Agent的“写代码”难度压下来**

Kimi K2.6建站Agent里，LLM写出来的典型查询经常在一条SQL里同时做多件事——按用户过滤、按标签筛选_（JSON字段）_、按向量相似度排序、按时间倒序。
...
但放在更大的坐标系里看，它是**一条正在形成的行业曲线上的一个点**。
...
![](https://mmbiz.qpic.cn/mmbiz_png/A6fTew8FFGGGt2iayNPickCVArSnxR9uke1Y3ozqr81Wwao0IW1In0ogOs78VRkHAeyCH62NQqT4AEllOHPPibHk7X9oibv9wfjPe6jjWnO9zTE/640?wx_fmt=png&from=appmsg)

###### **△**来自Dify官网

**今年，Kimi K2.6把TiDB用到了更复杂的场景****——Agent直接向终端用户交付数据库驱动的完整应用。**

![](https://mmbiz.qpic.cn/mmbiz_jpg/A6fTew8FFGEcJnnj9z7sm4jbPwPlRu8ibDvweNsgx90e2icnuPKVCPGNQiaUAE1VpPeC7rTPz3utxcHGIKd4tl4JF4gjEZX5tzO5hVBjNwmzd4/640?wx_fmt=jpeg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/A6fTew8FFGHnXZpZib08KbG4ay6fgDfLBjiaL1f3292f42liaoTy8VUjP2hjlSCen8jzFbVD8NGickfVWXEkK1uH2UY8th8dRhNFfF3PvibzpUK4/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/A6fTew8FFGGl6RmZeRN8mThb6HMnkqfjvX9GDstoNUO1yAEqI6PPw90JlaicMa6edr7ibib8icLmN53WiaSjDjAmUD1qWjEtnK8sCIQPAIUkt6Rc/640?wx_fmt=png&from=appmsg)

###### **△**图片由AI生成

Agent作为新一代应用的核心计算单位，它需要的不只是一个数据库，还需要持久化工作产物的**storage**、维持跨session上下文的**memory层**，未来还会有更多组件。
