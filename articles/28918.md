...
url: "https://mp.weixin.qq.com/s/kZrryL8_fxfq2pSFw6LSqg"
publish_time: "2026-07-07 17:31:34"
account_name: "机器之心"
read_count: 11908
like_count: 180
share_count: 1484
see_count: 111
comment_count: 1
stats_updated_at: "2026-07-09T02:51:47.168Z"
is_unavailable: 0
snapshot_reason: "网络同步补齐"
topics: "[\"Harness工程\",\"AI自我提升\"]"
entities: "[\"翁荔\"]"
---
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqHaKSzy7g1u3HVmGvQNpj65UTM2auBl6AOtBVI2cnL6muiadFWKA46MNZ9qicxDmxicFVsJzgsK7M8Hlx1ibFszvP11AI4pwuHUPsQ/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqEUyZJJUc8gmRtCq9pxEsibBkpZ2yiavSn0orEd0Licnc1MLCUcglhCJK032pOIFm3vmSNRqRUO4MAYTicrhj3ZjTr9Dc4LGJcG4Z0/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqGQibENicKHYooPrXB2fLCsgqIg0IYqkiauVaqO2ECvsv1aib9FLwJ1n5sIhqFwH8gA0YG79RjC36QhcVHvJXKIaKQUVtEyoIOyhco/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqH5MKHiaeHflh6Eff9G5kg6lZslQVddI3vOdRCpfaD7icNjiaHxb3LV6nFibNFE88ic5mqUpRGo2Y6LKIsdmr7th08My0PGa9Ak4oWQ/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqF0PsxMcoWHpVUjM6N0WGB0ZFXYsvg4jLhHvR53WAtQGVmIjwIq6u5uEEfibZ3YsRwYIYNC0nYAE3d2Q7HhqUsxx81SibOY3zwUQ/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqH1Uk7vQUUyuCsCd4iaaDZoogmZrk3gXoZicVXOmo6JbibvBCDJmibN26jk405aj9MngEfvpRMtBw5o4gF24FL4o17XPyXzhcuXOsE/640?wx_fmt=png&from=appmsg)
...
一个 MCE 技能 s ∈ S 定义了一个上下文函数 c\_s = (ρ\_s, F\_s)，将输入 x 映射为上下文 c = F\_s(x; ρ\_s)，其中：

  

*   ρ\_s = {ρ\_1, ..., ρ\_m} 是静态组件（提示词、知识库、代码库）。
    
*   F\_s = {F\_1, ..., F\_k} 是动态算子（搜索、筛选、过滤、格式化）。
    

  

这是一个双层优化问题：在给定技能 s 的前提下，先在训练数据上找到最优的上下文 ![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqF6ibn5uf4lnLjHrO8G585pG7F3CpahltvFM0EiboWBxuAAQ3fkjwSEvmao3ZDsoXvw3XddAchic9zia590FSrz7Id0icPox0VibafrU/640?wx_fmt=png&from=appmsg)；外层循环则负责在验证集上寻找能带来最佳表现的最优技能 s\*：

  

![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqFbfrZcvf8q32INCibUDAR0zxibf2hafZXee6qic4PtsDNUibd4KfmbibsKibyPv56dTTnCnHUCSRSibIT34r5RlAYQe0iceJ5LWq25tRc/640?wx_fmt=png&from=appmsg)

  

技能数据库会追踪历史上出现过的技能、上下文函数以及评估指标：![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqGN1OptRzKVjYVBvVrAX9AKf3tDzghic3grViaV4XtXdxpI3bnvYdxfxoW0vuNs4QSjbYb4gJuvkTexRaa40HOguNfFRtdkg7oYU/640?wx_fmt=png&from=appmsg)元层面的智能体会对已有的技能执行智能体化的交叉（crossover）操作，针对给定任务 τ 生成一个新的技能：![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqG3icSyBZcwRdyqRqEJnKlicCiaCTsmAjEp9yVbPRc9Xfosibsucjqd25ZLdhTYn8ENXvSacaPicliaLiaA5ABa3520SjlTJO0GK5TLug/640?wx_fmt=png&from=appmsg)

  

随后，基础层面的上下文工程智能体会执行这个技能 s\_k，并在该技能的指导下，根据执行反馈 R\_k 学习出对应的上下文函数：![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqF9FFWeGyPGnPibicsatyibjQJ7xNsrWMsTe8CGly3d2XiaPbvlPPich6SDsN9KyZIJ4hica7rfyL0yGBggTib0oEHwk4MDRUJFQaApzQ/640?wx_fmt=png&from=appmsg)

  

![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqEtibIvgTreuSiaYL5NwwBFEVkPUQtzpLmAACR418kR4TkvBdUECTW57635D4MiatOXZtlEfOCiaMSiaobgmau0L1SclWcx8JFS4IaY/640?wx_fmt=png&from=appmsg)

元上下文工程（MCE）的框架示意图：元层面的技能演化在各种上下文管理机制之间进行搜索，而基础层面则负责优化具体任务的上下文。（图片来源：Ye et al. 2026）

  

MCE 并不像 ACE 那样强制规定一套上下文结构的启发式规则。它使用自由形式的技能来存储某项任务中最重要的知识，并将技能和「以技能为条件的上下文」一起迭代演化。在具体实现上，一个上下文函数 c 被实例化为专属目录下的一组文件，既包括静态部分（skill.md），也包括动态部分（上下文和数据执行轨迹）。元层面和基础层面的优化都是在具备标准工具集 ![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqHYPLLZO0M44onhaVeyrSNdgfnVove9X4j0YJiagVxEibh9ZTg2F4Wz22k5mBpUcNk3Pia8IoP5iaCzEObmf9NXIE8FfibLuibWc1aX0/640?wx_fmt=png&from=appmsg) 的智能体编程环境中执行的。
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqEaeKicPxQBCib5icicjz959lewlm9xIRc2cnyqaP2eZTmYTB531YoWicm5w05oCykeMLiaka4k5ghESictKZStiaLDNECVrZj3fFXeKQE/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqER2njmmKm3Ij73wJUJiaiaMNHdDfrUBOuWTVgh6XbUgV2Ger4ARIV2r0EibsSaDbtCBjWAicP94IGHre07gdsxfZJGS1NzIGgfOFk/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqE3w1bx13TB0cGd51vKCUTYYs1Quxs9MMJlmNUJde6LPIqCK0xeP4gaPA7Z6sfJkTyZqULehDkKnFmMGb5Uswh8sEnLM45BSkw/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqEBsejrBKHibAWApueM5xKyqZcG7vlwacd6tqZU01iaMQrZibbszlwvGE9npBkTNjMyyfVSJ8iam2icGppziaDLkLjJMric1bHL0wArKE/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqE399ZJKPicNfJPOKPm5VibfaZw9Dhnvufu5nHrPFpib7vqcbk4zsmoRYfKicibclyQicc2CDQHNMVqVs0coPOgcODzp7yB2qibiaFy9S0/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqFp8p8ASLibE1JrH6ao2NQsFBS1LOibxbD1CdMP6vic28lNXPYRZC21IdXWTmWMyyVNzH6aV8bG0ARQicfk2CyVicvLvkaqaumN21TI/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqGWPgHXbxWs7DIkbh2yB90Gp9FkvX9Ny1LrzntukAicXtEKuHaAzeCkmBNlsfRmZAOd3XU7QpmjibQI6nsKHQqOnP2a1PXacXhFc/640?wx_fmt=png&from=appmsg)
...
自学习优化器（Self-Taught Optimizer，简称 STOP；Zelikman et al. 2023）是递归式脚手架改进的早期范例之一。在第 t=0 步的种子改进器 I\_0，接收一个初始方案 s、一个效用函数 u，以及一个黑盒语言模型 M，并返回一个改进后的方案 s'，也就是 s' = I(u, s; M)——STOP 的目标不是直接去改进方案 s 本身，而是去改进这个改进器 I 自身。
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqEFI38Qg8kgBSlqxMpogZZDYUIfrjNwTlZyhzwDOyF3EYPkQpf0m721lvwa2DNbemjZ3spUGYQ7e3r0H6YmZ5OBZU13e592Qzc/640?wx_fmt=png&from=appmsg)

  

由于改进这个改进器函数本身就是一个优化问题，我们可以依据元效用衡量出的表现，递归地得到一个新版本的改进器 I\_t，通过一次基于 I\_(t-1) 的自我提升式更新来实现：![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqE5pg70VldB64pfnMricd9A6N2x8KySrBUPIFa3k6EIh4rIUYddMlzCbvIbtCvIuOhC2vic3ia7MUPCwoQKbEmkzbWyjhPVBmrQj8/640?wx_fmt=png&from=appmsg)

  

![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqEX16j7MFgqic0DHaAZM5Au4A5XC8Tibk1eXmdqB1v7TR7Gz0my3Bj0uXvwMQ381LqdjF2TnBG3Wfcz5StymHokINrySvic1jicFVI/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqGLNHcNsic8AFNrPXh2yp6I0wqKqvevq3VDYhaT3EXvg0FRZmEkpGMHaExHqY1FlfBwW6gM9vZexemkTs78jPfVhasnRy6Px3rc/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqFfiafcxMibQGPnXLorXtm8MnKbcVvbuCFDicyfJZoB6aYTAw2WHqmFSic8FphibHt0NV3Z1uUwkC504QPdayUT7QicNzvZmTvz3G3kc/640?wx_fmt=png&from=appmsg)
...
*   用当前 Harness h\_t 在各项任务上进行评估，并收集执行轨迹以供分析。
...
3. 提议验证：验证并合并合格的修改方案，生成新的 Harness h\_(t+1)。

  

*   候选修改方案会在 held-in 数据集 D\_in（用于检验该弱点是否已被解决）和 held-out 数据集 D\_out（用于检查是否引入了其他未知问题）上，通过回归测试进行评估。
    
*   只有在 held-in 和 held-out 数据上都没有出现回归的候选方案，才会被接受。
    
*   被接受的候选方案会被合并，用以更新 Harness；被拒绝的候选方案则会被记录下来，但不会改变当前正在使用的 Harness h\_t。
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqHr1nkFqCuanc5jYyu6XJe6TibwfMFJpz2P6N2dUFM3dGbyfCNAd7yNdbSqRxCXkvdSYpFR2n9yWuETqnYdBNAUH3gbyibUImEj8/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqFscCaXtQsZa32ibtzzkQDYU2dZ6PMLPRYeYCfFupKFTibLLC3Zb3ib0vqfbiaFt5ia8eYHgWdpuKDgHE2I7AprZKLWkp7hBlpzFyEs/640?wx_fmt=png&from=appmsg)
...
3.  被选中的父代智能体会检查自己的基准评估日志，随后针对自己的 Harness 代码库提出改进方案，从而生成一个新版本的编程智能体。代码编辑通过两个基础工具来实现：（1）bash（参数：<bash\_command>）；（2）editor（参数：view/create/edit <file\_path>）。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqEZ3jn2OFyVkw84HncicwXSVN7T3qicDj3HAU9SuFaHAdj9WOzhmMy5JuH8h7Bu3vsPibruJhAyr5ibodiaicFa99CWEyUMB6C8XsTQ0/640?wx_fmt=png&from=appmsg)
...
Trehan & Chopra（2026）测试了 LLM 能否在极简的脚手架和基础工具（即 read\_file、write\_file、llm\_search、list\_files）支持下，从一个研究想法一路走到一篇论文。每个想法都配有一个专属的工作空间，供智能体在其中生成和读取文档，作为上下文的一部分。他们在三个领域（世界模型、多智能体强化学习、AI 安全与对齐）中进行了实验，每个领域都包含 45 到 50 篇高质量的种子文献，用以启发新的研究想法。最终只有四个想法被人类专家选中、进入完整流水线，而其中只有一个被完整执行成了一篇论文。他们在实验中观察到了六种反复出现的失败模式：
...
*   评估指标 fast\_p，指的是生成的内核中「既正确、又比基线更快」的比例。
