...
see_count: 20
comment_count: 0
stats_updated_at: "2026-07-08T10:06:52.845Z"
is_unavailable: 0
snapshot_reason: "网络同步补齐"
topics: "[\"多模态Agent\",\"医学分割\",\"SOTA\"]"
entities: "[\"浙大\",\"上海AI Lab\"]"
---
...
![](https://mmbiz.qpic.cn/mmbiz_png/A6fTew8FFGEqXIn6P4TyLe4TDlFzIiaPYc6tjOQkCYjEOiafcp8ELMCwrDy3b3Bkgia42icJoCfKEp6DsWNemUIswjF9TNwp8mOBBOkTicUjyHmg/640?wx_fmt=png&from=appmsg)
...
更深层的问题在于，为了赋予MLLM分割能力，现有方法普遍引入隐式分割token_（如<SEG> ）_并与外部pixel decoder联合微调——
...
*   将分割任务建模为多步马尔可夫决策过程_（MDP）_ ，以交错的文本推理与点击动作替代隐式token，保留MLLM完整的语言推理能力，同时实现自主多轮迭代精化。
...
*   行动（Action） ：模型输出结构化的空间点击指令，包含三个要素：分割目标类别_（Target）_、点击属性_（正/负，Attribute）_、归一化图像坐标_（Coordinate\_2d）_。模型可在一步中同时输出多个点击。
...
![](https://mmbiz.qpic.cn/mmbiz_jpg/A6fTew8FFGHL0ypicEtK6cOLicCZDiay6y2AYCw25Wvu8fbQwoTEZib3t7xqWrH19acNrPINOyOnMH2Xb6Kvzv7Hn0F51qNv86la30lB3h6yXVc/640?wx_fmt=jpeg&from=appmsg)
...
研究团队利用BiomedParseData_（340万图像-mask对， 9种成像模态）_开发了自动轨迹生成算法：
...
![](https://mmbiz.qpic.cn/mmbiz_png/A6fTew8FFGHqxIThwrRZU4qyeRNySA6gPEr5ia15xCp3Bg9yEXtm4fAwJichWYXaHO2CUWUvfdLkKMGDib9NBLgsYS5qXQOuBpOmxtBvGSqjgU/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_jpg/A6fTew8FFGGk1mwMxSEOnyI6PBI7dFT3Vfibf1T3Nm66NBia2cl0LBxug9x7rlw8Tk3nMicQicF5fqHZ8u0AGmqXzKx6PX0NJJnRiboibPgo4Oictg/640?wx_fmt=jpeg&from=appmsg)

实验结果
----

实验结果显示，IBISAgemt在域内测试集、域外泛化集_（MeCOVQA-G+，涵盖5种成像模态）_和自建私有数据集（1K CT/MRI/病理， 7类癌症）三个benchmark上， 均大幅领先所有对比方法。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_jpg/A6fTew8FFGFRu6w2MibASDibwbVbKKpG6hTY8QGw2PuBGGLMjMUgt65uPQ0h2qgm16XIfrdicyXqfIvAv5EVibBI8lxuB7mMMTh8ShV8ODicOlWI/640?wx_fmt=jpeg&from=appmsg)

### 与工具增强Agent的对比实验

相比同样调用MedSAM2的工具增强型Agent_（GPT-4o 、LLaVA-Med 、HuatuoGPT-Vision等）_， IBISAgent在域外集和私有数据集上仍保持显著领先，充分说明多轮交互式推理带来的增益远超简单的工具调用。

![](https://mmbiz.qpic.cn/mmbiz_jpg/A6fTew8FFGGJa8gmOPYuo9xrgVXQgRRdcruR3rW5m9Kb39rggEWYAJPeKOCicvh6gjy5uRCzeEda9mCx6QyzF6XRsibDsqzZQzY6ZcBLmC1yA/640?wx_fmt=jpeg&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_jpg/A6fTew8FFGGWFKyym3Dh5yB7ic7sicByfbeyOaRibFUUsZ4MPIfR43iaTY8kQcmvL6icyPEp2LCLliapUPa4fjjTAKaBkp7YdUPtxPgUkht4ibUu00/640?wx_fmt=jpeg&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_jpg/A6fTew8FFGH4B5QtxHbbM5uWrRlg8XyicMBYNe3Lj8McTuIMiaLYf84fa7fLHlIibzhkkoF7x4Nwkp6QV5tm39cmT3DyEGiasmwGJ0aplpYT8Pk/640?wx_fmt=jpeg&from=appmsg)
