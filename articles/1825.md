...
publish_time: "2026-01-12 05:21:12"
account_name: "量子位"
read_count: 28781
like_count: 151
share_count: 1062
see_count: 72
comment_count: 8
stats_updated_at: "2026-07-04T12:35:32.596Z"
is_unavailable: 0
snapshot_reason: "网络同步补齐"
topics: "[\"具身模型\",\"开源模型\",\"竞赛夺冠\"]"
entities: "[\"千寻\",\"Spirit v1.5\",\"RoboChallenge\",\"Pi0.5\"]"
---
...
刚刚，来自**千寻智能**的具身智能基础模型**Spirit v1.5**，在**RoboChallenge**真机评测榜上，以总分**66.09**，成功率**50.33%**的成绩，超越美国明星公司Physical Intelligence的**Pi0.5** _（π0.5）_，登顶榜首。

![](https://mmbiz.qpic.cn/mmbiz_png/YicUhk5aAGtAfhzgHBzUT6ibynmAmcEhb01MSX4N8Hh9Wpia8bXibuIiaEkXdHIzv6xaBG90rkFnjVCA4L6xKBzBSNQ/640?wx_fmt=png&from=appmsg)

**基于多样化的预训练数据采集范式**，Spirit v1.5在插花、把水果放进篮子、挂牙刷杯等多项任务中，拿下第一，刷新榜单纪录。

![](https://mmbiz.qpic.cn/mmbiz_png/YicUhk5aAGtAfhzgHBzUT6ibynmAmcEhb0dYGjZU6FkFs3RVAvnSYGA6rql5RPcgWSbUr1CKc80K5tx4OBCxI3gA/640?wx_fmt=png&from=appmsg)

经此一役，Spirit v1.5不仅是RoboChallenge自去年10月上线以来，**首个击败baseline模型Pi0.5的国产具身模型**，同时也是**首个在RoboChallenge上成功率超过50%的具身智能模型**。
...
也正如当年CV的开源景象，Spirit v1.5同步开源了**基模权重、推理代码以及使用样例**，方便后续的研究者复现和进一步探索。
...
其中 “寻找绿盒” 任务堪称高光时刻——Spirit v1.5直接将成功率拉至**90%**。

![](https://mmbiz.qpic.cn/mmbiz_jpg/YicUhk5aAGtAfhzgHBzUT6ibynmAmcEhb09g3DYDUBoJNWvyVicd1GiaXvLfrRBa4DtS4rorNKRb0iaOzW5GJFGK3kQ/640?wx_fmt=jpeg&from=appmsg)

演示画面中，它能快速从一堆彩色方块中锁定绿色目标，稳稳放入指定篮子，整个过程干脆利落，没有丝毫拖泥带水。

在水果入篮任务中，Spirit v1.5更是以80%的成功率，领先Pi0.5整整一倍。

![](https://mmbiz.qpic.cn/mmbiz_jpg/YicUhk5aAGtAfhzgHBzUT6ibynmAmcEhb06YF1qStmpAWXcGk4yZ3Yb6zYPic1NWibJFEPwtQrYqQU0enF2aNUrJgw/640?wx_fmt=jpeg&from=appmsg)

在演示中，Spirit v1.5能够轻松地拿起香蕉，放进篮子_（下图经5倍加速）_。

![](https://mmbiz.qpic.cn/mmbiz_gif/YicUhk5aAGtAfhzgHBzUT6ibynmAmcEhb0hnhaNNE0MaXwT2Cuna5tXMGuOFddlTa0bM4S75Qk7w8KlEiaKVaJlng/640?wx_fmt=gif&from=appmsg)

而在插花任务中，虽然两款模型成功率均 50%，但实际执行中，Spirit v1.5的稳定性碾压Pi0.5，没有出现极端的失败案例。

![](https://mmbiz.qpic.cn/mmbiz_png/YicUhk5aAGtAfhzgHBzUT6ibynmAmcEhb0hgEa1AlFElDTnKgpYwFnzpxOvUXFUTt39EWsTAPO8I1TrGcnwXTbGA/640?wx_fmt=png&from=appmsg)

![](https://mmbiz.qpic.cn/mmbiz_png/YicUhk5aAGtAfhzgHBzUT6ibynmAmcEhb093z9iaKz4Z6VL7hNAzLib89mIMAGFWOqPBF4ZCuXcXD4jnek6O6Bt14Q/640?wx_fmt=png&from=appmsg)

_（左为Spirit v1.5，右为Pi0.5）_
...
RoboChallenge由**Dexmal原力灵机**联合**Hugging Face**发起，是首个在真实物理环境中，由真实机器人执行操作的大规模、多任务基准测试。
...
除上述任务设置外，RoboChallenge它的核心创新，在于**Remote Robot Paradigm（远程机器人范式）**：

参赛者在本地运行模型，只需通过HTTP接口向机器人发送控制指令，机器人被视作一个可远程调用的“外设”。

![](https://mmbiz.qpic.cn/mmbiz_png/YicUhk5aAGtAfhzgHBzUT6ibynmAmcEhb0oPjftVQ7XhZLYEKoibYHCzeDiczv4Notec3BuCrxxX9W671oXedlvdfw/640?wx_fmt=png&from=appmsg)

这一设计显著降低了参赛门槛，同时避免了复杂环境配置带来的不确定性，使不同团队的算法能够在**同一套真实硬件条件下**接受统一评测。
...
![](https://mmbiz.qpic.cn/mmbiz_png/YicUhk5aAGtAfhzgHBzUT6ibynmAmcEhb0TqUCyPjRa3RcKtONoIGrq9EtTyxj4Px9cic1CRVlicvEwjgHWOSyYnfQ/640?wx_fmt=png&from=appmsg)

_（注：RoboChallenge区分任务特定与通用型两种训练协议：前者针对单一任务单独训练，后者用少量混合数据训练一个多任务统一模型。榜单中带有/multi 后缀的模型，如Pi0.5/multi，遵循的正是这一更具挑战性的通用型设定。）_

也正是在这一背景下，Spirit v1.5此次选择同步开源，其意义不仅在于成绩本身，也契合了RoboChallenge鼓励通过**可复现、可验证**的方式，共同推动具身智能基准向前发展的初衷。
...
例如**Open X-Embodiment** 、**Agibot**和**RoboCOIN**等具身模型训练的主流数据集。

###### **![](https://mmbiz.qpic.cn/mmbiz_png/YicUhk5aAGtAfhzgHBzUT6ibynmAmcEhb09YVWL8oDxZqTkbrbA5qnjvg78JnPNsyfRVTJECaIZRO8ZajcqeiaianA/640?wx_fmt=png&from=appmsg)△**Open X-Embodiment
...
在工程层面，这一策略同样带来了可观收益：人均有效采集时长提升约 **200%**，对算法专家深度介入的需求降低约**60%**。

而在实验验证中，这种以多样性为核心的数据策略，同样得到了印证。

一方面，在**RoboChallenge Table30**的真机评测中，Spirit v1.5已经在整体能力层面证明了该范式是成立的_（相关结果已在前文展开）_。

另一方面，在消融实验中，研究团队在**预训练数据规模完全一致**的前提下，对比了两种策略：
...
结果显示，多样化预训练的模型在新任务上的**微调效率显著更高**：在达到相同性能时，所需迭代次数减少约**40%**。

![](https://mmbiz.qpic.cn/mmbiz_png/YicUhk5aAGtAfhzgHBzUT6ibynmAmcEhb0Qia7eevLrv6duNibKPib8RV675okU5MXujtawfFnmokydXryeicaf2OMVA/640?wx_fmt=png&from=appmsg)

进一步扩大多样化数据规模后，模型的验证误差仍在持续下降，并未出现明显的早期饱和现象。

![](https://mmbiz.qpic.cn/mmbiz_png/YicUhk5aAGtAfhzgHBzUT6ibynmAmcEhb0phnGYzwlVMrBr3RHH7yEEORfkbeMCtPsgAMHHicm9pKZj6NdFceSRgg/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/YicUhk5aAGtAfhzgHBzUT6ibynmAmcEhb03niaQ89ibKCiavcW2sxWHUBYHcSzPWcrGM9kcd1zO1pOojcogVFibHfg2g/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/YicUhk5aAGtAfhzgHBzUT6ibynmAmcEhb0cpEUoN8UM0HZdZBDQ9kUYRuH0a98ZC6slPFxIhTGibxZdHGoDhz3DPA/640?wx_fmt=png&from=appmsg)
...
_开源地址：_  
_Code: https://github.com/Spirit-AI-Team/spirit-v1.5_  
_Model: https://huggingface.co/Spirit-AI-robotics/Spirit-v1.5_  
_Blog：https://www.spirit-ai.com/en/blog/spirit-v1-5_
