...
see_count: 64
comment_count: 0
stats_updated_at: "2026-07-08T09:59:41.927Z"
is_unavailable: 0
snapshot_reason: "网络同步补齐"
topics: "[\"KV Cache\",\"模型压缩\"]"
entities: "[\"Together AI\",\"TurboQuant\"]"
---
...
长上下文模型越来越能“记”，但真正让它们跑到线上时，最先顶不住的往往不是算力，而是**KV Cache**。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/A6fTew8FFGFvIRZT0fibsZ7tY7fecI6lyOhficVVwL0060FRicgZTv85CWF7EJg4Q0iauDaQKzIpCEQSoeHHEeJ7bQMuT9gEuerRU08o3q2SrrA/640?wx_fmt=png&from=appmsg)
...
在约2.28 effective bits per KV element的预算下，OSCAR仍能**接近BF16**；在Qwen3-4B-Thinking上，相比全层3-bit K/V TurboQuant，最高**提升40.1分**。
...
###### **![](https://mmbiz.qpic.cn/sz_mmbiz_png/A6fTew8FFGFWPW1BgGbcXb0PoynEk4aSUiahzB03icFHCV5QTSPQf9COeic8hmiaXfY9fHWsfibhQMj3Mb1ibmzww4GLNrBZJjIEt3KNAzId1fdkw/640?wx_fmt=png&from=appmsg)  
**
...
具体到**Key**，量化误差会通过QKᵀ进入attention logits，因此OSCAR使用query covariance，也就是QᵀQ，来决定Key的旋转方向。

具体到**Value**，误差会先被attention score加权，再进入attention 输出，因此OSCAR使用score-weighted value covariance，也就是VᵀSᵀSV，来决定Value的旋转方向。
...
###### **![](https://mmbiz.qpic.cn/mmbiz_png/A6fTew8FFGGB4qrtUWafibe8y2t3s87Y5awyv7ESbCdTibssfoCAJ4xr4Kw2112ZfNUY6lLH0cSRKx5icLfwmQGWGqxicdj0Qibd5wl7VETotUgA/640?wx_fmt=png&from=appmsg)  
**

###### **△**从离线校准到在线推理的pipeline

OSCAR的另一个关键点，是**它没有停留在离线量化评测里**。
...
结果显示，在约2.28BPE下，OSCAR的精度仍然**非常接近BF16**。

以**Qwen3-4B-Thinking**为例：
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/A6fTew8FFGElu54h2HBYM4kibA5lSVSQHBs528AxWWZr4cYl93Ar5wjAeneJAv1wj6tGgOURTWksB5RECK3tcLQV49tdPf3mTRDtQbRgxqNI/640?wx_fmt=png&from=appmsg)

这组结果背后的含义，比单个榜单数字更重要：

当任务真正依赖长链推理、代码生成和数学推导时，低比特KV Cache的核心瓶颈不是“能不能压”，而是**压缩误差会不会破坏attention的关键路径**。

OSCAR的优势，正是让接近2-bit的预算仍然守住推理质量。

论文还专门看了**AIME25**这个高难数学推理任务，并加入KIVI-KV2、Kitty和OSCAR的对比。由于KIVI和Kitty没有可直接用于long context run的framework支持，论文选取了它们唯一在32K下汇报的AIME25结果。

在Qwen3-8B上，OSCAR以2.38 BPE达到66.67，几乎追平BF16的66.00，并明显高于KIVI-KV2与Kitty。

在Qwen3-32B上，OSCAR达到74.00，略高于BF16的72.59，也超过Kitty的69.26。

![](https://mmbiz.qpic.cn/sz_mmbiz_png/A6fTew8FFGEdOMx5yxXric4Qax53erTSGggPCULFLfqxBwKUjTs2QUKicWr6Bx1xic9sxhYtQx01ticIUN8QcQybbC5RIYaWvAFcCYrVhv9qlZk/640?wx_fmt=png&from=appmsg)

这说明，OSCAR的优势不只体现在与TurboQuant的比较中。在现有KV Cache量化方法里，它也能以接近2-bit的预算守住**困难数学推理能力**。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/A6fTew8FFGECDnUgIscJrZTFuRibZEIckicQrnpftoR88EuJQ6KbEVzo5wVubtV9LIqFscaacj5Mdkms3TVlNJzMDDMJRoGM02VCrry02H8Xk/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/A6fTew8FFGFCksDX3hiayEQwSFdTtSETnf8btTMay5ZJjoQPo9ia1FgAeac66piaAQjmQ771jp7zkMv5cgk1gGWwd23Pdia2hNNecBsw1r0Wnx8/640?wx_fmt=png&from=appmsg)
...
OSCAR的设定**更接近真实服务**：历史KV主体统一使用INT2，只在sink和recent两个很小窗口保留BF16。
...
OSCAR带来的关键启发是：2-bit KV Cache如果要真正上线，旋转不能只追求“有”，而要**对准attention**。
...
_论文链接：https://arxiv.org/abs/2605.17757_  
_项目主页：https://oscar-quantize.github.io/_  
_代码链接：https://github.com/FutureMLS-Lab/OSCAR_  
_ModelScope链接：https://modelscope.cn/models/togethercomputer/OSCAR-RotationZoo_  
_HuggingFace链接：https://huggingface.co/Zhongzhu/OSCAR-RotationZoo_  
