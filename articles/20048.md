...
url: "https://mp.weixin.qq.com/s/yOX1QZTHs54J-CBg-rKUig"
publish_time: "2026-05-11 05:39:16"
account_name: "宝玉AI"
read_count: 5712
like_count: 167
share_count: 1076
see_count: 75
comment_count: 4
stats_updated_at: "2026-07-09T05:46:28.345Z"
is_unavailable: 0
snapshot_reason: "网络同步补齐"
topics: "[\"智能体\",\"架构设计\",\"技术原理\"]"
entities: "[\"Harness\"]"
---
...
虽然这个术语在 2026 年初才正式确立，但其核心理念早已存在。**Harness**是包裹在大语言模型之外的完整软件架构：它包括编排循环、工具、记忆、上下文管理、状态持久化、错误处理和护栏（Guardrails）。Anthropic 在其 Claude Code 文档中直截了当地指出：SDK（软件开发工具包）就是“驱动 Claude Code 的智能体 Agent Harness”。OpenAI 的 Codex 团队也使用了同样的说法，明确将“智能体”和“Harness”等同，指代那些让大语言模型真正发挥作用的非模型架构。
...
![](https://mmbiz.qpic.cn/mmbiz_jpg/ZlwY8rlDcFMMIqnCobF3ePoewapSMLf9ia28nXaibjPic9IHKicT1UuAsXoiaYX2ibuX9SmGlfh4IpiaxCIQsqb3lfPxuZOXmDQjy8icVoweWLlRdd8/640?from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_jpg/ZlwY8rlDcFN5OXibT1icMUrh9icqVibjXib8YmibDI3djT4kfrnlJISDqf8Zqv9tvMXFLMotbrXCJ35saVYibEyPibIuEYB97ssHCKUdW20gG3jorGo/640?from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_jpg/ZlwY8rlDcFNuaRqbnSKkfCAuaVBjgcDRgVtiaicoRe3Sv4zibj4LOsudHn2oyxBFl3bA3tKEyE4NnuCQrZ7F1LsibSL3icIKYa69NjuLajaXSzEQ/640?from=appmsg)

1.  1. **第一步（提示词组装）**：Harness 构建完整的输入信息。
    
2.  2. **第二步（模型推理）**：组装好的内容发送给模型 API，模型生成 Token：可能是文本，也可能是工具调用请求。
    
3.  3. **第三步（输出分类）**：如果没有工具调用，循环结束；如果有，进入执行阶段。
    
4.  4. **第四步（工具执行）**：Harness 校验参数、检查权限，在沙箱中运行并捕获结果。
    
5.  5. **第五步（结果打包）**：将结果格式化为模型可读的消息，捕获错误以便模型自愈。
    
6.  6. **第六步（上下文更新）**：将结果追加到历史记录，必要时触发压缩。
    
7.  7. **第七步（循环）**：返回第一步，直到满足退出条件。
    

现实中的框架是如何实现的
============

![](https://mmbiz.qpic.cn/mmbiz_jpg/ZlwY8rlDcFOWeMLcMAZ6ETYoibSf8NCg5N9llwQYRu1nqJXgAqOOOpFhY5Tic5xL5FcxTHX5n4P6icUomaT1ia2ZUCpHydIUKyXf3zglibBxbKSI/640?from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_jpg/ZlwY8rlDcFPTHBTz5u4RL9RcvbibELoScrCsRWMk1C8GZuktU8BhOeJsyAmKe5LY5KZtgKWWvqFqd1E3MNUtt5OkIefst0zIrHd8VVLoW5Lo/640?from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_jpg/ZlwY8rlDcFNyHvKqy3fMT48ibG8VicwEwGVDmGib9zFrVBvgf1HVVJ0Z8ibX4FC3CRa4tH4a4PyiaXz0B547x7YjweeibJdpeaszZS0156v9LhAsc/640?from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_jpg/ZlwY8rlDcFOUfbnsEtovxZrNuuBvibq9ZVY4ydZUQPj4bOqSGX2h3fsiaxic0KbLS2Hhuq22Bfceia3dsaVXqouI1v3C9gfrr6qLPe6xq3GLFl0/640?from=appmsg)

1.  1. **单智能体 vs. 多智能体**：官方建议：先充分挖掘单智能体的潜力。多智能体会带来额外的开销和信息损耗。
    
2.  2. **ReAct vs. 先规划后执行**：ReAct 灵活但成本高；“先规划后执行”速度更快。
    
3.  3. **上下文管理策略**：是总结对话，还是动态加载？
    
4.  4. **验证循环设计**：是用硬性的代码测试，还是用另一个 LLM 来打分？
    
5.  5. **权限与安全架构**：是追求速度自动批准，还是追求安全步步确认？
    
6.  6. **工具范围管理**：工具不是越多越好。暴露当前步骤所需的最小工具集往往效果最佳。
    
7.  7. **Harness 的厚度**：多少逻辑写死在系统里，多少逻辑留给模型发挥？
...
原文：https://x.com/akshay\_pachaar/status/2041146899319971922
