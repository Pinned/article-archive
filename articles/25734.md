---
id: 25734
title: "过程比结果重要：一个不给标准答案的调参框架，让Agent自己把数据库性能榨出来"
url: "https://mp.weixin.qq.com/s/e0IufepaMKp5aAvXowJD2w"
publish_time: "2026-06-22 16:27:54"
account_name: "量子位"
read_count: 4405
like_count: 35
share_count: 190
see_count: 20
comment_count: 0
stats_updated_at: "2026-07-08T09:56:33.832Z"
is_unavailable: 0
...
为应对这一挑战，**中国科学院软件研究所**的智能软件研究中心和基础软件与系统重点实验室团队，联合推出**PerfEvolve**框架。

![](https://mmbiz.qpic.cn/mmbiz_png/A6fTew8FFGERKrYC6qJXMa6FTjUuOyTY5lO8Zzh2Ubj9MuEYn0ALbXr2Cc8DXqWn70kzOIEV3QhTsLqhGMFsunYgOlUfQm7rkKrlcDdVp8c/640?wx_fmt=png&from=appmsg)

它不教LLM死记参数答案，而是把枯燥的静态调参文档，转化为Agent可直接执行、可自主落地的**过程化调参技能**——
...
**1、静态文档严重滞后，标准答案早已过时。**  
市面上绝大多数数据库参数推荐值，都源自多年前的旧版本、旧硬件环境。最典型的就是**PostgreSQL**，不少核心参数的官方建议值还停留在机械硬盘时代。
...
最经典的就是PostgreSQL里：shared\_buffers = 25% RAM。

这个推荐值几乎所有DBA都见过，但不同负载对缓存策略的需求大相径庭。

实验发现，仅这一个参数配置不合适，就可能带来**5%-16%**的性能损失。
...
传统文档给的是结论式指令，比如：shared\_buffers建议设置为内存的25%。
...
为了让Agent的调参能力标准化、可落地，PerfEvolve将零散的调参知识梳理成**结构化可执行技能文档**。
...
![](https://mmbiz.qpic.cn/mmbiz_png/A6fTew8FFGHrdjLNepUm84UXTKt7svgjibLvBEYQRecZfZybQ2wAbFj1p20x6Ld4DjicJI09VUDQnebcJ4pibcYAX5RISPzvRribNqicsOzFSZfc/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/A6fTew8FFGGrthN7QmepWS9JWL252Osn4HxdjlibglNBWD0rn6SkCbN1UEBKrAme6FtMedR1MEb8iaU767NNcnbsVZ6jSkBXhbjcIXdypb50Y/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/A6fTew8FFGGOnlQXQB9LkYhGJGnkn1ZNtBmiaUbR4XSCBXgcoZP0mVPdFuNPMW1RCa4wQ3QckiaM72STBKDg3CG3jhicUeia7KsPzhENU6Hiax7E/640?wx_fmt=png&from=appmsg)
...
数据显示，相较传统文档驱动调优方案，PerfEvolve最高实现**58.9%**的性能提升。

![](https://mmbiz.qpic.cn/sz_mmbiz_png/A6fTew8FFGEic7UUrjAwARcx3NRheTyI8iacczm9ueSibBibunnkrGaHP0wFMPpa6qicmLv5icbepokEabAbh5qyUeL0Pia96oKUBHywpFqTYYcNxA/640?wx_fmt=png&from=appmsg)

更亮眼的是，PerfEvolve无需替换原有调优系统，可直接作为通用知识模块无缝接入各类主流Agent调优框架。

以GPTuner为例：原版模型会从文档中抽取57个参数做贝叶斯优化，不仅搜索空间大，还有**19%-32%**的概率调出崩溃、严重退化的异常配置，实验有效率极低。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/A6fTew8FFGECZdn3CdibdVShmD2nhiaibaALjCNhWs5fyd9f3nb0micJpicsHByCmsEfTaibJ5FlwT9baJq4AQUzkWicgsK6JGrbpfBicibiaMGTXiaZs0/640?wx_fmt=png&from=appmsg)
...
_论文链接：https://arxiv.org/abs/2605.19988_  
_GitHub：https://github.com/ISCAS-OSLab/PerfEvolve_
