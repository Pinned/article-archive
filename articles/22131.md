...
see_count: 12
comment_count: 0
stats_updated_at: "2026-07-09T03:00:12.227Z"
is_unavailable: 0
snapshot_reason: "网络同步补齐"
topics: "[\"智能体\",\"并行推理\",\"ICML\"]"
entities: "[]"
---
...
更重要的是，NPR 的突破点不只是「并行生成的工程技巧」，而是提出了一套「自蒸馏 + 并行强化学习」三阶段训练范式，并配套专门的并行推理引擎，目标是让并行推理从外部推理策略变为模型的原生认知能力。

  

![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqGr0YpI8GibbnicCzlkibP5vbXGqPbIia79ljyf8TQYcklWtg1qE0j6Fs4ibun8dIsibrQol2fn7dWKuKuC1bpQVmkZtGe8P14Vbl7MQ/640?wx_fmt=png&from=appmsg)
...
人们对语言智能体（Language Agent）的研究已经把关注从「单一思维链扩展」推广到了「多步深度推理」。模型能够进行更深层次的推理令人兴奋，但未来的超级智能真正需要的，是能更广泛地并行探索多条可能思考路径 —— 也就是在一次推理过程中同时尝试多种解法，然后再把结果合并校验。类似 MapReduce \[2\] 的分而治之思路对进一步扩展智能体的测试时计算 \[3\] 的能力边界至关重要，但想把它天然地整合进一个智能体中，存在巨大的挑战。
...
像 Multiverse \[4\] 这类方法虽能实现并行，但高度依赖于强教师模型蒸馏出的示例，无法通过自举的方式扩展自身的智能边界。学生模型不过是在模仿教师的串行拓扑并把它「塞入」并行格式，结果是把教师的局限也一并继承，短时间内难以产生新的、模型本身固有的并行策略 —— 达到了目前的「智能瓶颈」。
...
人们对智能体的期待，正在从「能多想一步」的单一思维链，升级到「能多维思考」的深度推理。未来更强的智能体，必须具备广泛探索多条思考路径的能力 —— 这很像经典的 MapReduce \[2\] 思想：把复杂问题拆开并行处理，再聚合结果完成全局最优的决策。
...
*   学生模型更多是在模仿教师的串行拓扑再「塞进并行格式」，
    
*   结果是把教师的局限一并继承，难以自举式扩展智能边界，
...
现有推理引擎、强化学习算法对「分支 — 聚合」结构往往缺乏原生支持：

  

*   推理引擎难以高效调度并行分支；
    
*   常用 RL 技术可能会截断或削弱触发并行结构的特殊控制词元梯度，从而阻碍模型学习严格的并行控制逻辑。
...
在具备并行表达能力后，NPR 用自蒸馏方式让模型用自己的生成结果反过来训练自己：

  

通过筛选与沉淀，让模型逐步内化「多分支探索 — 相互印证 — 汇总收敛」的推理规律，而不是照搬教师的串行偏好与局限。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqFriasMepMmrQYeXofhxrBM6yJVcHzdPxPBMYU6h3UtsS7YLTicdQRsZ6z0zibVA2xe4sqm11u90yBVj067h6JLYPtOUbMMggWURc/640?wx_fmt=png&from=appmsg)
...
*   方法：对 NPR-ZERO 进行拒绝采样 \[7\] 并应用严格的筛选器（必须同时满足 「格式合规」与「答案正确」），保留自蒸馏的并行推理轨迹，然后在此之上做冷启动的并行 SFT 预热微调，同时引入并行注意力掩码（Parallel Attention Mask）与并行位置编码（Parallel Positional Encoding），让模型内部能够支持并行分支的独立计算（并实现 KV Cache 重用以避免重复计算）。
...
*   方法：提出并实现 Parallel-Aware Policy Optimization (PAPO) —— 对并行语义做专门修改的策略优化方法：使用并行 Rollout 的 NPR-Engine 推理引擎以保证结构正确性、在批次层级进行优势归一化、保留特殊结构化 Token 的梯度并放弃重要性采样以维持稳定的 On-Policy 同策略梯度更新 \[8\]。PAPO 能直接在并行计算图内优化分支策略，从不断地试错中学会有效的问题拆解与合并策略。
    

  

![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqH44tUE47RB1MeFZScliariaNmE48IEq3GHSgIda8u3tzUPVFojib28acfyp2JzZt0TWQwHMyqnyyEXtiafDmIibxzsJLbtoTI5xyibw/640?wx_fmt=png&from=appmsg)
...
从 NPR-ZERO 生成大量并行格式的候选轨迹后，采用两条硬性筛选规则只保留高质量样本进入 D\_accept：
...
为在单次前向传递中同时存在多条 Reasoning Path，NPR 采用 Multiverse 风格的并行注意力掩码与专门设计的并行位置编码（对应论文给出的 Algorithm 2 伪代码），保证不同分支互相隔离但共享上下文 KV Cache \[8\]，从而实现 KV Cache 重用并避免每条分支重复计算上下文代价。该编码亦允许通过标签 Token 标明分支 / 步骤 / 指南块，便于引擎解析。
...
1.  并行 Rollout：使用 NPR-Engine 产生严格遵守并行 Schema 的轨迹，保证样本合法。
    
2.  结构化过滤：格式违规样本在进入优化前被剔除，奖励退化为纯准确性（+1 / −1）。
    
3.  批次级优势归一化（Batch-level Normalization）：由于格式违规样本被移除，组内方差塌缩，因此用更大范围（batch 内多组）统计标准差来稳定优势估计。
    
4.  保留特殊 Token 的梯度 & 放弃重要性采样：为防止触发并行结构的特殊标签被裁剪掉，PAPO 在 Token 级别保留梯度流；同时放弃重要性采样，采用严格的 On-policy Objective，避免重采样比带来的不稳定。
    

  

![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqF7pYRDle6wC8MatycwCytGexT2LhU5USWALMSTBmA9v8WOLKAhL0CbAV7MDqZcgPTkHmD38sP5rLluloBkwM5qMNXWdfdkrHc/640?wx_fmt=png&from=appmsg)
...
*   分支感知的 Token 累积策略：把全局 Token 预算从「只看最长分支」改为 「按活跃分支因子累计」，避免超出 max\_new\_tokens。
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqGShzvb0knl0ceIzoibVMHuZqGicrX7Km9qALlLHAFzVxDnd36icnibDvgZ2QC3WQgSTT4p5ichCm82KsKHVxpneWl81mqTzfCkQzicg/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqHNqmRhDKYGegg0mTNOLRUcW6SvyuEicK0wB39y6KIhjVIWYibnmMs0Z9fVPxkTiarQYb4SkOYwEcb1xjmkkJ7p0icAZrUjwmB4icmk/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqEicHg2HvtDfUrEryDcpj3pDVuZWNgxicMsYrhGcTWW87zEC4fwQG6R0jriaK4SKsEJAbBwm9XI7ibPzQdYQhial1Me8nA8LjBSRPS0/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqGicxQHD1offnLW3Odx9UlwVP6iaGXArcR5krGzs2tfico6NWRTPO6eTU4HpeALqKq5ibj2icRO8C0l9aGSl3AyammvYoFL7eZR2EsI/640?wx_fmt=png&from=appmsg)

  

![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqEmc5mYKj3gtUhVbmBnSlfkwtNkqbKM5hYqW2pDDqo5NQX2azLMkrouvmxm6j5vaicvytEmkTTYqcTYFMRsRbUsDwZic1icDO41Ss/640?wx_fmt=png&from=appmsg)
