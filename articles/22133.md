...
see_count: 33
comment_count: 1
stats_updated_at: "2026-07-09T03:00:14.452Z"
is_unavailable: 0
snapshot_reason: "网络同步补齐"
topics: "[\"大模型\",\"深度思考\",\"ICML\"]"
entities: "[]"
---

# ICML 2026 | 只用少量Thinking Tokens，大模型依然能深度思考

![](https://mmbiz.qpic.cn/sz_mmbiz_jpg/5L8bhP5dIqHvOw60LsQNF0cIia6JbxSxFib5HSic73mmK8ZZic0Gpvf3Ay1bwicnwia2KibBic4Ld0klnStxphKzAKpzabqIK7OicsNYLnibwlBppqeXw/640?wx_fmt=webp&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqGX3hrr4yjy5nqP9PwCR0B2iaE6y9YK7CksmziaXwdwQ2BlPpGzDt0DZnBFGbic46TTlZVPo0FDYI5tvyfLF3FBn5xKrrKkoRibCOc/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqEIIxhicyHOSQdoQAVFCrrzplTKiciaNgYiafhqDZSgH9WNibG7xEZZHy4hrS0iarP9Sa9hZFiarx3omfeseSggBC7m74h2TPWpMXaXw0/640?wx_fmt=png&from=appmsg)
...
> <Thinking\_of\_Summary> <Thinking\_of\_Caption> <Thinking\_of\_Reasoning>， 结论：这张图片展示了一辆黑色 BMW M3 在路上驰骋。
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqG66SsPR0fI6zCFicawZkTImTGAgUAsANgUUQ0NqdRDbaJy4Lc4DFe2iccbxicurwybSP1S3UEt1OFUN08IuZf9mjobfBJ8ck2w2k/640?wx_fmt=png&from=appmsg)
...
直接把完整 CoT 一次性压缩成少量 token 是很困难的，因为模型可能会丢失大量推理信息。为此，Heima 采用了 progressive distillation 的训练策略。具体来说，模型并不是一次性把所有推理阶段都替换成 thinking tokens，而是逐阶段进行压缩。这种渐进式训练可以让模型更平滑地从 “显式文本推理” 过渡到 “隐式抽象 token 推理”，避免一次性压缩带来的性能下降。论文明确提出，Heima 会逐步将每个 CoT stage 蒸馏为 thinking token，而不是一次性完成所有阶段的蒸馏。
...
为此，本文设计了 adaptive interpreter。它的作用是把 thinking tokens 映射回可变长度的文本序列，从而重建模型的推理过程，并进一步分析压缩引入的信息差距。这一步非常关键，因为它让 Heima 不只是一个 “把推理藏起来” 的加速方法，而是提供了一种分析和验证隐式推理质量的机制。如果 interpreter 能够在没有视觉输入的情况下，从 thinking tokens 中重建出与原始 CoT 接近的推理过程，就说明这些 tokens 确实保留了足够多的推理信息。换句话说，Heima 一方面让模型推理更快，另一方面又通过 interpreter 尽可能保留可解释性。
...
从信息论角度看，thinking tokens 可以看作是原始 CoT 的压缩表示。记原始 CoT 为 ![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqGibIPeIiacKSO3P69jVuZQ1aOT465ONmRQp1Sic2e7T4SeftWhW1I1sEOUvRFKeXTzbRXUiaFfbF5TzJKdiaKJgwCfTwMYZPfDY9Uo/640?wx_fmt=png&from=appmsg)，压缩后的 thinking tokens 为 T ，则 T=f (X,C) ，也就是 thinking tokens 依赖于输入的问题 X  和原始 CoT C。

  

因此，由数据处理不等式可知，在给定输入 X 后， T 中关于答案 Y 的信息不会超过原始 ![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqGkQR5qAD7yCpgIAKlsQdEO0iaNQL5tdl1iaDtbOaWiaO3c7ovPGTfAoGoIN8x4n9lv87VwaAZUNCtHABS1jeck06Ueg3POibgBquk/640?wx_fmt=png&from=appmsg)。这说明压缩不可能凭空创造新的推理信息，只能保留原始 CoT 中已有的信息。但这并不意味着压缩一定会破坏推理能力。只要 ![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqFsPwzvXV3aqTtZ9AqfBXNmhCbia866J5aYMgNy3WC0KwsM56IqaIV1I4BRepPC2ibQ2L4fFHdpkPUDr1vP91o20EMoukaXs3XGk/640?wx_fmt=png&from=appmsg)，thinking tokens 就仍然保留了对最终答案有用的任务相关信息。压缩真正损失的是：![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqGffWDwtUUZ14icdpHIBKcsugAqcF1m3I9CFTmFbMuiaXlaicV2lqFUKY6TF75wWUO1ebUf54EHiaDSIwXy7yzROHU62nrkF37b8as/640?wx_fmt=png&from=appmsg)。 这个条件信息量表示：已经知道输入和 thinking tokens 后，原始 CoT 还能额外提供多少关于答案的信息。如果这个 gap 很小，说明 thinking tokens 已经捕捉到了原始 CoT 中最关键的推理信息。这也是我们后续设计 interpreter 来量化这个损失信息量的动机。
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqEMDkMBnicL5x6d1Jud7ib29lmgjIJpNFpQCu3ou3hG14Zjccg2PYuEJMx3Hda5GIKfCkibOnTEKiaJmX88rYxSssDnKYaagS0oAg8/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqEVYkMCZuKpVV1x4ojPkgiaxQNAJ0mDhibzZppG6GOqSODtlDdDrFDJY6WTq7QcVEFV1cPBSE3PzXqibLLlW4blHnp8nEZNAQvEFQ/640?wx_fmt=png&from=appmsg)
...
*   第一，推理更高效。通过大幅减少生成 token 数，Heima 直接降低了自回归解码开销。
    
*   第二，能力保持较好。在多个多模态推理 benchmark 上，Heima 能够在显著压缩 token 的同时保持接近甚至超过原始 CoT 的性能。
    
*   第三，仍具备可解释性分析机制。通过 adaptive interpreter，Heima 可以将 hidden thinking tokens 重新映射为文本推理过程，从而分析压缩后的隐式表示是否保留了足够的推理信息。
