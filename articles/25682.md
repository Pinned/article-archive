---
id: 25682
title: "大模型搜索总偷懒？IQuest等联合推出FORT，30B开源搜索Agent刷新同规模SOTA"
url: "https://mp.weixin.qq.com/s/2bE3HQOC-cbEl4OYsQntBQ"
publish_time: "2026-06-27 10:28:12"
account_name: "量子位"
read_count: 1754
like_count: 22
share_count: 102
see_count: 8
comment_count: 0
stats_updated_at: "2026-07-08T09:55:37.662Z"
is_unavailable: 0
snapshot_reason: "网络同步补齐"
topics: "[\"模型搜索\",\"智能体\",\"开源\"]"
entities: "[\"IQuest\",\"FORT\"]"
---
...
来自至知创新研究院（IQuest Research）、中国人民大学高瓴人工智能学院、KAUST等机构的研究团队提出了**FORT**，一个**面向Deep Search Agent的shortcut-resistant training-data synthesis framework**。

与单纯经验式构造复杂问题不同，**FORT首先从理论上建模Deep Search任务中的shortcut collapse，再将这些风险转化为数据合成中的显式控制项**。

基于FORT生成的搜索轨迹，研究团队训练得到FORT-Searcher。该模型使用Qwen3-30B-A3B-Thinking-2507作为基座，**仅通过监督微调（SFT）训练**，就在多个挑战性Deep Search基准上取得了**同规模开源Agent中最优的整体表现**。

![](https://mmbiz.qpic.cn/mmbiz_png/A6fTew8FFGGVicUYibw3FQxlstHVRiczxvkwdgsGjS6tVCxcDj3pcGOfzmMO7cwmKZia5o4FYKVr0icNkeic2hib0Q1FewBicZNwheO9o1OVM6UibVBc/640?wx_fmt=png&from=appmsg)  
![](https://mmbiz.qpic.cn/mmbiz_png/A6fTew8FFGFp19gwavWbMxHMG9VK80RKOuZ3wDEIF1ShMuDRB2jqmoaRERm2ciaz1JD8dcMNqgwXmSKblwvqjJfO0ZyjcDbtRQ1QH7MiaMJKk/640?wx_fmt=png&from=appmsg)
...
论文将Deep Search问题形式化为**多约束检索任务**。完整约束集合可以唯一确定答案，但模型在实际求解时不一定需要验证所有约束。只要某个较小的约束子集已经足以识别答案，并且这个子集可以通过很短的搜索路径被验证，任务难度就会塌缩。

论文将这条低成本答案识别路径称为**cheapest identifying route**。
...
FORT的理论贡献在于，它不是孤立地观察这些现象，而是把它们**放入统一的shortcut-aware difficulty framework中**，解释它们如何降低真实搜索成本，并指导后续数据合成。

FORT：把理论风险变成数据构造控制项
-------------------

FORT的目标不是简单把问题做长，而是**系统性减少cheap identifying route**，让模型更难通过单个线索、单个网页、题面常量或参数知识提前获得答案。

整个合成流程包括四个阶段：Graph Initialization、Graph Construction、Question Formulation和Adversarial Refinement。

![](https://mmbiz.qpic.cn/mmbiz_png/A6fTew8FFGFgYFojtVh1y4tsXEXhY0NUeDoRqaeYls08GqJQFjf9m59HqibK9sRxLNGCe6lJXFbyTMz4PVOIg3wIjJxVk2hdibH4ic2TtHqxIA/640?wx_fmt=png&from=appmsg)
...
除了实体选择，FORT还尽可能使用cycle-based initialization，而不是简单线性链条。线性链条在问题生成时容易暴露中间实体，导致后续query从一开始就能执行；cycle seed则可以**让关系表达更间接，减少exposed constants风险**。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/A6fTew8FFGFGpb7H84lf6Vwobd61rhnlbwfkoIo98r245ibverkE26RiaEINSmEhhGSIibvouuByMzHm2libYfjj9Lcfn08N6Mq5oy5d4jTC5NY/640?wx_fmt=png&from=appmsg)
...
这种设计**让每个clue都有贡献，但又避免任何单个clue过强**。

### 隐藏中间实体，模糊精确常量

在Question Formulation阶段，FORT会将answer-bearing subgraph渲染成自然语言问题。

为了减少exposed constants，FORT会**隐藏中间实体名称**，用关系描述替代直接命名。例如，不直接给出某个人物、作品或机构名称，而是通过其属性、关系或事件间接描述。

对于必须出现的数值、日期或名称，FORT会进行**exact-value fuzzing**。比如，将精确年份、数字或日期改写成范围、类别、数字特征或间接约束。这样可以避免模型直接复制题面中的精确字符串去搜索，同时保证问题仍然真实、可验证。

![](https://mmbiz.qpic.cn/sz_mmbiz_png/A6fTew8FFGGukYfvdNIqxsnuiaewPQwwvpKgoy12ldRtAQEDkuejzMoAX17a6s3GcQxjf8aGAD6fEEQqTtreKGvwYunCdOIWmJ5ibPHC0ytw0/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/A6fTew8FFGFFNt3icXAZ3CVibZY764BUibeFHM0TX59Srqqgu1dGz1HXxibZyS31sS3MXZlH7dmCStPibLicyVQtBdODriac5E96LTAwfKQujEw4ps/640?wx_fmt=png&from=appmsg)

###### **△**FORT-Searcher与其他Deep Search Agent的主结果对比。

实验结果显示，FORT-Searcher在comparable-size open-source agents中取得**最高Overall分数66.2**，超过MiroThinker-1.7-mini的64.6和Qwen3.5-35B-A3B的59.9。
...
更重要的是，FORT-Searcher只使用SFT训练，推理时约激活3B参数，却在BrowseComp上**超过多个更大规模开源Agent**；在BrowseComp-ZH上，也取得了表中所有开源Agent的**最高结果**。
...
因此，论文引入了一组**trajectory signatures**：

*   Solving cost（![](https://mmbiz.qpic.cn/sz_mmbiz_jpg/A6fTew8FFGFnNPq2mZQZ76PFwcWaYWtXumEmOMZsy1ibUnCcd4h5MhvbFn8yEMicxGRmff9CernwF7QoCFRAy5bYg5Z4FFgicQPhMjOMQanAxo/640?wx_fmt=jpeg&from=appmsg)）：模型完成任务所需的总体搜索成本；
    
*   Answer hit time（![](https://mmbiz.qpic.cn/sz_mmbiz_jpg/A6fTew8FFGFPsTQB5YdsDkKpkxmszKMcAticNlpS6A5JPlhr2dDP1Zy4WibJicwsiacibCpp0ViacaicqJKSvv6QibbkMPzF6v5PWMBmJoP8oZQ61f8/640?wx_fmt=jpeg&from=appmsg)）：答案首次出现在轨迹中的轮次；
    
*   Prior-shortcut rate（![](https://mmbiz.qpic.cn/sz_mmbiz_jpg/A6fTew8FFGH9JvsEjOlg3mn54S9ica1axicmA8CXTUGDtQ6cszgU8arpsx06A8hzHCfQep9ploOicmdv8rZMNfNUqI0MkkiaHlYMibwjg8xoa5wY/640?wx_fmt=jpeg&from=appmsg)）：模型在证据充分出现前提前提出答案的比例。
    

![](https://mmbiz.qpic.cn/sz_mmbiz_png/A6fTew8FFGE19nMGQJwLiaNSpr6D59X4fdz4X6icKMWr4wrpK3Waeugh3Yx2EulIYX7G6jjHytLQMUodVE3fd88dcWEhQlEOzycibFg0G0fY5k/640?wx_fmt=png&from=appmsg)
...
这说明FORT并不是简单诱导模型多搜几步，而是在**构造阶段减少cheap identifying route**，使答案发现过程本身更难被绕过。

进一步的training-data difficulty analysis也验证了这一点。

![](https://mmbiz.qpic.cn/mmbiz_png/A6fTew8FFGGU5YA9CX0uQEoyrOicbxpGo4pGibA0COn0ffiaeumkp2rictaFoRvWOcWoVKyGE4tERXKHQ1LSSibt41ICt2KB6UKhC6uKF7PdxicicQ/640?wx_fmt=png&from=appmsg)
...
_论文题目：FORT-Searcher:Synthesizing Shortcut-Resistant Search Tasks for Training Deep Search Agents  
arXiv：https://arxiv.org/abs/2606.12087  
GitHub：https://github.com/RUCAIBox/FORT-Searcher_  
