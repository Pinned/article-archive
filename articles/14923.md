...
see_count: 131
comment_count: 0
stats_updated_at: "2026-07-09T02:32:25.109Z"
is_unavailable: 0
snapshot_reason: "网络同步补齐"
topics: "[\"意图检测\",\"永久记忆\"]"
entities: "[\"NUS\",\"NTU\",\"Pask\"]"
---
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/Rvq8Ow69CYXM3sLflURdPIHavkkxRFlsZsmFhMEaoqRFKOaPnxS5wx08ibfwkbSlos0k6NkwFJJOW4QiccHcibstxkfNRLhJrxejas5JHlhPjk/640?wx_fmt=png&from=appmsg)
...
##### **【新智元导读】让AI像助手一样主动帮助，才是我们心中AGI的样子。主动智能体的概念已经被多次提出，但都很难做到可以真正在生活中落地。现有的工作都还停留在概念层面，无法解决复杂世界中所要求的实时性、深度、和记忆等问题。 南洋理工大学谢之非团队提出Pask，使用「底层小模型流式意图检测」+ 「上层Agents执行」架构，实现首个能够做到实时、有深度、基于个人全局记忆自进化的主动智能体。  
**
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/Rvq8Ow69CYWELicNso7Vcb36xoxZ3KicRvtAVYCpUYjGPU3rQ9w3emUNiadicQuVKpMG0MvnZIBweBrQEffmicVkaxLUStElDjMfiap69nlpdQh8w/640?wx_fmt=png&from=appmsg)

论文链接：https://arxiv.org/abs/2604.08000

![](https://mmbiz.qpic.cn/mmbiz_png/Rvq8Ow69CYULiaYK7UiakbBLk7mKGTcsjQOAUF8OmXs8EQo3B4MyOUDwVex44nml51xFwCTGX1ThYFyRh2U0oZ7IUPqI1SnSicETFk9vcFZYxc/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/Rvq8Ow69CYWt98GZ7CXzzRLXS7a3twJGAVaWnQ6Hljiask1ClfGgc2Ts4LACBOwaXgebeLj1C9gxvAaGfXnHQZd11oABGTNPnnPKRjYnrvKg/640?wx_fmt=png&from=appmsg)

首先需要确定：主动AI需要哪些「组件」？

研究人员提出了一个通用范式，将被动模型转换为主动智能，包含三个相互作用的模块：**需求检测（DD）**、**长期记忆（MM）** 和**主动系统（PAS）**。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/Rvq8Ow69CYUZwoKg059rqTBWykAgrLVIgiaegJnOe17sJVuPDnd6wdN3jAhmVbmDicbkgNh4YrRofQrD17ymMXmlQkkGt61Se2ibq40aBF9uKs/640?wx_fmt=png&from=appmsg)

主动AI做过头就是信息骚扰。

一个好的主动AI，必须在**实时性、精准度、触发频率**上做到精准平衡。但不幸的是，准确率、记忆查询与实时响应，这三个特性天生相互矛盾。

更大的挑战在于：这件事完全无法用传统 Agent 的方式来做。如果整个流程需要在2秒内完成，留给意图检测的时间最多只有1秒，甚至不够完成一次API调用。  

而意图推理，到记忆查询，至少需要10秒。  

Proactive AI不是简单的Agents机制能做的事，研究人员从语音和视频的端到端流式模型获得灵感，选择了「模型+Agents」实现路径，即重新训一个在「文本流」上实时运行的意图检测模型，构建出了**IntentFlow，**接收文本化的多模态信息流和用户记忆，自主判断人类当下需要什么。
...
![](https://mmbiz.qpic.cn/mmbiz_png/Rvq8Ow69CYWkOIeRfcYibTqI2Trsql8W5NiakbvbkNqccLd6f8zdicsibjdhsjx0dXyT84kREbBlmOLtYQPfjECnYxvpseJDGPeVc9eNBl43fTI/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/Rvq8Ow69CYUOvMSU2P1umP1stIWgebbCp5q3kMB7mEJsCAdnP3RaEFku3tgTHHQaGcm6uKI7sme2CrTm7lD9OU5o0m8UzdTXuDVrgYuS2zg/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/Rvq8Ow69CYVlCWWFXtPn0lfovnXymaVWZ3VtCQjvKVl717tW08NaMXoQdFnicP5V9LlLYlRia6VYqqAJRw0fU8g1H4tK81cesO5FEyNiaYNaqw/640?wx_fmt=png&from=appmsg)

![](https://mmbiz.qpic.cn/mmbiz_png/Rvq8Ow69CYXwbeeYu5X3sDiboDLrcU1Zicicvbylcx9hVkBo99j3jH8ibsVIsmS10kzicPZIDf5jul1c6iaBgeO3afXbXhZwcbcZmzcwGp7v6L6Bg/640?wx_fmt=png&from=appmsg)

Pask在**学习、工作、日常**三大领域的十类任务上进行了测试，效果与闭源模型加思考链的水平相当。
...
![](https://mmbiz.qpic.cn/mmbiz_png/Rvq8Ow69CYWtrDngFRaib0ARPI6MpEjSkwdH6H8RFibsRvKTUOqAcicva2TPblw0VWWRHfKYx2aUUiawrbDMUVyM8WwRXDMtpqFG3spwRBZJhqM/640?wx_fmt=png&from=appmsg)
...
随后与颜水成老师一拍即合，选择离开清华大学加入NUS LV\_Lab，成为其回归学术界后第一个博士生。 论文通讯作者为颜水成老师、苗春燕老师、叶德珩老师。
