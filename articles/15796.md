...
see_count: 11
comment_count: 0
stats_updated_at: "2026-07-09T02:31:54.404Z"
is_unavailable: 0
snapshot_reason: "网络同步补齐"
topics: "[\"AI小镇\",\"生成世界\",\"虚拟智能\"]"
entities: "[]"
---
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/Rvq8Ow69CYXhXPLbvficMXXZ4B91mta80Y1qfZGWEcyqVcUXDsbfxQha1zPxkicp8WZnH2vUq517svAeQic6Z1EyulxgZYDzUB0ZrCPCW2KPuE/640?wx_fmt=png&from=appmsg)
...
##### **【新智元导读】2023年斯坦福「AI小镇」火了，后续也诞生了大量类似的热门项目，但所有这类项目都有一个共同瓶颈——世界是人工搭建的，固定的。最近，一位独立开发者用10天婚假爆肝了一个项目WorldX：输入一句话、5分钟，一个完整的AI世界就诞生了——地图、角色、动画、人设全部自动生成，AI角色们自主在其中生活、对话、形成记忆、产生戏剧性的涌现行为。**
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/Rvq8Ow69CYVsfOicuwk4fToDw8hepzXSjeRj5CS7wO8RaIc9b65zENlDiaWic0Wp9AvoCoBdficXrygPQR7WCm4WdnWtibBsDkgOr7Sfiabq13qW0/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/Rvq8Ow69CYWs7VHeSuJG6jccBktdviclGhcGE88jJ8bRoWRmLa6Gm9ZYFucFIbYXcibhQLAhjJNRQTXQKlMBbUuUegicS1icndgM9VjkFDRpUFI/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/Rvq8Ow69CYXhnWVursJec8wRU7PRFmQt5ovjXaDbB0q1s3UEC8FLJIuU5AbQicp27QyIziblicK5tIYrWxPxsdgkah2aiaia41jjIhL0fVElf6FQ/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/Rvq8Ow69CYWre596VgZjICXhzOicrFu7X8eaaaSaLOVrlsKQdWWpic2slPq9C4dQ2al6GtZAKCk7ibPTwSF1RMUCh0S9g5lYz5GETnMpCdhmS0/640?wx_fmt=png&from=appmsg)
...
*   每个AI角色在地图上**自由走动、决策、互动**
    
*   角色头顶会冒出**对话气泡**和**内心独白OS**（小偷和捕快客套时心里想着「她不会已经知道了吧……」——节目效果直接拉满）
    
*   角色们**形成记忆**、**产生情绪**、**做出反思**——一天结束时它们会在脑海里回顾今天发生的事
    
*   世界有**真实的时间流转**，凌晨1点夜市会自动收摊，第二天19点又重新开张
...
*   **全局广播**事件——「突然下大雨了」，所有角色下次决策时都会知道
    
*   给特定角色**耳语/托梦**——你突然想起当年捕快办过的一桩旧案……
    
*   实时**修改角色人设**——把「老实木讷」改成「心机深沉」，看世界走向会怎么变
    
*   把任意角色「拉出来」，**和它进行架空对话**——而且不影响主世界进程
    

![](https://mmbiz.qpic.cn/sz_mmbiz_png/Rvq8Ow69CYXPApSaZTCx0tmZn2lqBeNN5jU2Rf8B3FMz4K99h8hrjibTmoG5OGUI7Na9EoTqBMNaVQZqOrPoXibsUcGNjvRPajw2Aicr6iaxId0/640?wx_fmt=png&from=appmsg)

更绝的是WorldX还做了**多时间线 + 历史回放**机制——同一个世界可以衍生出多条时间线，看相同的初始条件下故事是否会走向同一个结局；任意一段历史也能像看录像一样被回放，让你不错过任何「名场面」。
...
![](https://mmbiz.qpic.cn/mmbiz_png/Rvq8Ow69CYVVH4ibKicW0ibvclblrMjGaic2MOZuWn1b7Qbl6LfucBTo4TJrLVYAjCk9C2NtjdgxkdHUutGsx4WHjhqq0gTaiaMw4Szd4MiaCYEBc/640?wx_fmt=png&from=appmsg)

这件事看起来简单——人一眼就能看出哪里能走、哪里是树木屋顶。但要让代码知道，意味着需要**精确到每个像素的坐标**。而文生图模型生成的地图，本质上就是一张「图片」，没有任何分层、标注、坐标信息。
...
![](https://mmbiz.qpic.cn/mmbiz_png/Rvq8Ow69CYXGh29yzA4R7iaSTib3Q1wS6xNbNZSgxbscLKclnlz6JdjQ0adUtsznzKkuTVRQeU46tD6OtAcvUIZn4JVSUG7XBxzwv3fhMJWvQ/640?wx_fmt=png&from=appmsg)

加网格辅助呢？作者也试了——给图片打上参考线，然后让 VLM 看着网格定位，再加自我审查循环不断纠偏。**有效，但只对建筑、可交互元素这种「小目标」非常勉强地能用**。对于「可行走区域」这种**大范围、不规则的区域标注**，几乎不可解。

![](https://mmbiz.qpic.cn/sz_mmbiz_png/Rvq8Ow69CYUIkS85diazJlfvKT74LQic4oETQMujBhcr5HAZqicyw3ZUlWmRkPEu1wCKP78ibKMjwnZWRciclHDvosRR6znElWayuOKuKKPmTFMM/640?wx_fmt=png&from=appmsg)
...
转折点来自一个「换位思考」的瞬间——_如果是一个真人，要在地图上把所有可行走区域标出来，他会怎么做？_

可以**直接拿水彩笔，把可行走区域涂出来**。这连小孩子都能做。

然后，**只要把「涂之前」和「涂之后」的两张图做像素级色差对比**，就能精确算出所有被涂抹区域的坐标。某个像素颜色变了，说明被涂抹了；颜色没变，就没被涂抹。代码层面，这是一段**完全确定性、不依赖任何AI**的计算。
...
作者把文生图大模型作为「识图工具」使用——把原图作为参考，让它**用半透明的青色覆盖出所有可行走区域**。

![](https://mmbiz.qpic.cn/sz_mmbiz_jpg/Rvq8Ow69CYUREu8UPibe587KkmfiaXbCnfZphangZjjYofiageHPAyLbaRMzufTm5HDgMaPnslWgxjmDeOia30ASeY4XiazGrgOrclJ373I2MbZ8/640?wx_fmt=jpeg&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/Rvq8Ow69CYVQnKS4M46nbukfbX19ziaTxS9ShvTvKXYlgjAtDxKo9cyNj4s5u1XqRVuSgLsTGL5qjkuXLpyvxDfE9DSIEpzLcbHPN9VHCGeA/640?wx_fmt=png&from=appmsg)
...
2.  **压缩工作图**（降低下游 token 开销）
    
3.  **三路并行**：功能区定位 + 可交互元素定位 + 可行走区域标注（每一路都有自己的审查重试）
    
4.  **像素级计算可行走网格**（纯 CV，不依赖 LLM）
    
5.  **坐标映射 + 拼装 Tiled JSON**（标准游戏地图格式，引擎可直接加载）
    

![](https://mmbiz.qpic.cn/mmbiz_png/Rvq8Ow69CYWjcXCic9jG067gc25vrFdbeoWbTmtoiaInnVibNmibiatF9YFJEXZjB66SMhatvRt66vCgicbichRwR6v5LTUBbAYicYL8baicr3cDqJns/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/Rvq8Ow69CYXQWtrDZtBdYGeflKr1KSfCibblB5RQH96kRrB1IMjPUx1gGTKfO8u8IP78jOGH2OINC2iaJnjRcEKrTtNgMTt9rYDTAX19ksv18/640?wx_fmt=png&from=appmsg)

引擎以**Tick**为基本时间单位运行（默认每Tick = 游戏内 30 分钟）。每个Tick内部都是一套精心编排的流程：

1.  **决策波（并行）**：所有可决策角色同时走「感知环境 → 构建动作菜单 → LLM决策」
    
2.  **对话调度（冲突消解）**：贪心算法分配，每个角色每 Tick 最多参与一个对话会话
    
3.  **动作执行**：非对话动作串行（避免共享状态冲突），对话会话并行
    
4.  **微反思（每模拟小时触发）**：从近期经历提炼洞察，调整情绪和关注焦点
    
5.  **跨日转场**：一天结束时触发深度反思、记忆衰减、状态快照
    

**记忆系统**作者用了一个透明的四维加权评分：
...
![](https://mmbiz.qpic.cn/mmbiz_png/Rvq8Ow69CYUN44JBvfnTUX6ZslmQcv1TtdLfzFEzrO1Cg80yPic2VYUC10dPGhpfK3UOMGKJLGCUjsH8jx33gFLxptltCX9u001pCmmldTVk/640?wx_fmt=png&from=appmsg)
...
_声明：此项目由本人在婚假10天中爆肝vibe coding完成😂，时间确实有限，还有很多待优化的点会后续实现_
