...
publish_time: "2026-04-30 13:36:04"
account_name: "机器之心"
read_count: 2687
like_count: 22
share_count: 144
see_count: 10
comment_count: 0
stats_updated_at: "2026-07-09T03:02:55.549Z"
is_unavailable: 0
snapshot_reason: "网络同步补齐"
topics: "[\"3D数据\",\"视频标注\",\"场景理解\"]"
entities: "[\"CVPR\",\"通研院\",\"SceneVerse++\"]"
---
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqGT663ShCHmtBdG6fuRf300aCaeqVeAM9vWzVrEcLFWqDZen4iaewSjhMylPMxp3fVzmCwwGIEqPJliapUlBmAXTia3iaetNtgXljM/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqErFDyib2gLHgicbcib3R6oF3POCF3GkkOLXhkf2xNrYq0ERoJbywfuq2fic0mibwRQqbhA2wHViaHPKae7fWZF3hHmiasmWZZlHuxyYI/640?wx_fmt=png&from=appmsg)
...
最终，从 8217 段互联网视频出发，团队得到 6687 个真实室内 3D 场景，规模已超越同类真实数据集，并且由于素材来自长视频，SceneVerse++ 天然包含多楼层、多房间、大范围的复杂场景，这是传统房间级或实验室级扫描数据所不具备的。

  

![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqHdNfdQSNJlII2RsZGPJbDlBFvV3mAnMmXfn4oQXY4nBLpn9zHQg7nbXtNAiamv7dT6F7C6Pr2UicicQ7XpwHPHtucPW93D4qUWBc/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqH3JUVudmFMEJyKlsFpXlvaP1X1H5zGzP0Tob0QWPicmOUYfaE7ud6IIpRuFmEVbFNgqC6YdzhvMxKT9Cys7tHb1WwIFWrJEF9w/640?wx_fmt=png&from=appmsg)
...
管线输出总计 632K 条空间 VQA 样本（391K 多选题 + 241K 填数题），可直接用于 VSI-Bench 格式下的训练与评估。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqFDSOKeTTVl0HgicMeBxiajsicg6bQ7OwTpEpoCiaMkb84al3oFbjO1iaxY4iaib5wgicyMT2icibomMXOoJ0Gp3ZshDokqD74r45PrPZ5D0/640?wx_fmt=png&from=appmsg)
...
该管线最终在 SceneVerse++ 上产出 9631 条轨迹、平均长度 12.8 米、平均 15 步，共 7189 个不同场景下的 21567 条指令，为 VLN 研究提供了大规模、高质量的真实视频轨迹资源。
...
作者以 SpatialLM（基于 MLLM，原始模型在 12000+ 室内合成场景上预训练）作为 3D 目标检测代表，以 Mask3D（基于图分割 segment 的实例分割模型）作为 3D 实例分割代表，分别在 ARKitScenes 与 ScanNet 两个真实世界基准上进行零样本与微调评估，所有模型架构保持一致。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqHpicxXmsCbV9adBHAvQdGnhgc2Aictu5RCaFPmPVdIG0AVribKV61MHD41TgyO7fGDj4pEV0OvBt7bjWMNtuZNVpmsw27p30Tr3s/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqH04zOwgmBStMMGEsufzLsk6qFTepbiaZ5hBHAAkNpcibqhODLrEpVyzqgDlQhcFS0I0rFkEVLYbk3et6GvSsxtrXarkj0t62ctw/640?wx_fmt=png&from=appmsg)
...
3D 空间视觉问答的评估在 VSI-Bench 上进行，作者在 Qwen2.5-VL-3B / 7B 两个规模上用 LoRA 微调，训练数据分为四组对照：
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqG2dkuvGzbNoHfKE2fjMiaceLxWyfAt9e1hjxy64H5TKHgFe8PpYNmHIbOhk7HkeYgGOiajnTJiaWDCndEzc57NBXklmdR7eMY95E/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqFLzmrn5bpHJHbhzHibKN6esDRwrk8JdKy1yg4RfvsECRjGT0eJj1AaM4qdBiajpS5pR71mJhsicra6ExiccmCamACNEoCB3a3ctsU/640?wx_fmt=png&from=appmsg)
...
评估基于标准 Room-to-Room (R2R) 基准（Matterport3D 环境），所有实验使用相同训练轮次以确保公平。指标包含 SR（成功率）、OS（Oracle 成功率）、SPL（路径长度加权成功率）、Dist（距目标距离）、PL（轨迹长度）。
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqEb7w9BDYk6JB3VTIdDpIE7UJX8kVIJDjSkQOQX4nA020ZamMbz4DdOldEdUBxbL37H4pvBbprHk3ObCQLQvXxSLbRyriaBq67Q/640?wx_fmt=png&from=appmsg)
...
感兴趣的读者可访问项目主页 https://sv-pp.github.io/ 获取更多实验细节与可视化资源。
