---
id: 19714
title: "ICML 2026 | 拒绝盲目猜token，阿里x浙大将投机解码带入弹性预算时代"
url: "https://mp.weixin.qq.com/s/LpnIXlqICLCsVw3LiSQxTg"
publish_time: "2026-05-13 10:00:34"
account_name: "机器之心"
read_count: 4699
like_count: 44
share_count: 262
see_count: 21
comment_count: 0
stats_updated_at: "2026-07-09T03:01:11.195Z"
is_unavailable: 0
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqEstTW1LLEadMuk7ibp9NeB2LxlttHAzoWEYy1ShQouU7vnmjZXYQtXL0xyiaJaibic4ywsjzeic6B7M3hWUv0My8gcP4Oup3RrfCXw/640?wx_fmt=png&from=appmsg)

  

为了解决这一问题，来自阿里 ATH 事业群 - 千问事业部 - 基础工程团队和浙江大学的研究者提出了 ECHO（Elastic Speculative Decoding with Sparse Gating for High-Concurrency Scenarios）。

  

  

![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqFC3pae0QE57ej38muImNmab9mibDOGcrpK29Zrc7gP1GicEngNuOx5eFUHw91xrZMqgPpbFkmPYOfD8j9hJGgZzPecj7m32zXHc/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqFB3YH9J6AsAMWplxdOxKb5SibcMTQuiaj9542INibtvjxAdQSTyibkFzOdJCtPbicy97SwuEQQibUiaP2MnibgUUH60MtGNZD6nL3LOGk/640?wx_fmt=png&from=appmsg)
...
论文观察到，当负载升高、batch 内待验证 token 数增加时，目标模型的 verification compute 会逐渐成为主瓶颈。以 LLaMA3.3-70B 为例，随着 batch size 增大，目标模型验证开销![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqEY4qwmnpzC0Ah3zjzuKMLvIXGYCUekNSaqBh3227MDOgVYDRSeRoCMbicibcL6qZaGYkUlAF84jxekhJw16GPlzVHoWweDYNJ9M/640?wx_fmt=png&from=appmsg)呈明显增长趋势，并逐渐超过单 token 自回归成本。对于 Qwen3-235B，EAGLE-3 在低并发下能够提升吞吐，但当 batch size 增大到 128 时，其吞吐甚至会低于 vanilla AR。
...
> 在一个 batch 内，将所有请求的候选 token 树看作统一的 Super-Tree，并在全局验证预算 K\_max 下弹性分配深度与宽度。

  

在每个投机解码 step 中，batch 内有 B 个并发请求。对于第 i 个请求，假设其构造的候选树包含 K\_i 个待验证 token，那么目标模型实际要验证的是整个 batch 中所有候选节点的并集。ECHO 对其施加全局约束：

  

![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqFnEAfIIEtIExxZcIj8Xp6VZciaJjya3sbkTtLiaJ5v2M0FBBRibYtd82JuDD6zcDDibJ9lRRNe0SrVTlygNu04MU5NCvuHpzmAoibA/640?wx_fmt=png&from=appmsg)

  

其中 K\_max 代表当前硬件和 serving 系统在 compute-bound 区间附近能够承受的验证上限。这样，投机解码就从「每个请求独立扩树」转变为「多个请求共享一个全局预算池”」。
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqEmb5urNM0GGkzaG1UpwicM6O3OHI8iaDsD22iaLZBQ2gf79tA6wic4ooADPCQCQ4dfhA4QxPBxhTWJcDM5AzJUc2dsjE2fzphBicZY/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqFQbT3zAGST4NyKRiazOBHhPKTNhhqHC27iaVHrEOUOoM8XINaVQD9QEBBZH80bHbQvGwn1KhBSlOfxlj0ySV41ic3ZuXCpE0OLgY/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqEXtfP8OPpm9VNGHYf9Rxgs1pwe5toUv9iaC6KIp68e6uGaklhv8JgbQ2O9XNWfBJFNn0WRCqQgTFFRSic1ZPzvvOAyEicU2NicyOs/640?wx_fmt=png&from=appmsg)
...
ECHO 在系统层面专门处理了这一点。论文将 ECHO 集成到工业级推理框架 SGLang 中，并通过 Flatten & Pack 将不同请求产生的非规则候选 token 树打包成 dense、kernel-compatible 的布局，再交给目标模型进行一次性验证。
...
在低负载 BS=1 的设置下，ECHO 在所有 benchmark 上达到 1.63×–5.35× 的 wall-time speedup。其中：
...
对于 Qwen3-235B 这类工业级大模型，verification compute 更早进入 compute-bound 区间，因此错误的预算分配会更快伤害吞吐。ECHO 通过将低置信度请求节省的 token 预算重新分配给高置信度请求，在 BS=256 时将吞吐从 2,803 tok/s 提升至 3,207 tok/s，提升 14.4%。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqHpfaf67qKAIdJY5sxxKfh7kb4vzwWjk5KqhlJibzkHheibZkmeH4lq0GrZ4WDiaUzPZoz3605VOGVfICvnF1hCWvXBnFR2iaS9xDg/640?wx_fmt=png&from=appmsg)
...
在 LLaMA3.1-8B、BS=256 设置下，Dense Gating 比 ECHO 低约 5% 吞吐；在 Qwen3-235B 上，ECHO 相比 Fixed Threshold 提升 5.3%（3,046 → 3,207 tok/s）。
