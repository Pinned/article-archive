...
url: "https://mp.weixin.qq.com/s/Wp9edClPZpu1H-mHJAyizQ"
...
see_count: 239
comment_count: 3
stats_updated_at: "2026-07-04T12:36:00.144Z"
is_unavailable: 0
...
RoboChallenge真机评测榜单上，来自**自变量机器人的端到端具身智能基础模型WALL-OSS**，以总分54.69、成功率35.33%的成绩，超越美国具身智能明星公司Physical Intelligence的pi0（π0），排名全球第二。

![](https://mmbiz.qpic.cn/mmbiz_png/YicUhk5aAGtAicMcwK0mUlOribS1QRQpgkkGcXgy1QCAKFEF747xmmZqGQSnY3SPPZSgic6QSImdOwWWAFUzOXkvYA/640?wx_fmt=png&from=appmsg)

在叠洗碗巾、挂口杯、按按钮、浇盆栽、移物入盒、开瓶器进抽屉等多个单任务中，WALL-OSS均拿下单项第一。

![](https://mmbiz.qpic.cn/mmbiz_png/YicUhk5aAGtAqFvhUzJ7gBb1xsEW1gXDmNaCestABQB36B0xkhGhJbH4CFg3okCq1S7UHAiaJ8DNLh7zxL2HtVHA/640?wx_fmt=png&from=appmsg)
...
相较于闭源模型的测评结果存在较大操作空间_（其性能可能源于对第三方模型的微调、接口层的特殊适配，或者存在黑箱内的未公开优化）_，模型本身的原生能力不容易被外界验证，开源模型的成绩建立在完全透明的代码与参数之上，其能力可被任何研究者复现、检验和深入研究。
...
![](https://mmbiz.qpic.cn/mmbiz_png/YicUhk5aAGtAqFvhUzJ7gBb1xsEW1gXDmLRiaA6oOdZoqBLgZvnMwS8bqIUdMsZqOa7fj7PAGic1b4WdEeia4k2Sxg/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_gif/YicUhk5aAGtAqFvhUzJ7gBb1xsEW1gXDm0duvgL3Fo4xjcSg4HhZiaFtrprwliaQ8FIAtEvuicicSSYhS4xg3ibCzDgQ/640?wx_fmt=gif&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_gif/YicUhk5aAGtAqFvhUzJ7gBb1xsEW1gXDmzCH9O4UAiaVJDuc81seBrAsKeLTxMmm2HibkloIE6jXJ5zqYY1dE0r1g/640?wx_fmt=gif&from=appmsg)

在相对简单一些的“连续按下三个按钮”任务中，WALL-OSS的优势更加明显，得分显著领先其它模型。

![](https://mmbiz.qpic.cn/mmbiz_png/YicUhk5aAGtAqFvhUzJ7gBb1xsEW1gXDm88E7mVgUib0AROdqEbsGIchA6U5tKb9NPsDySVrfpE8crHOOiaaGzIxQ/640?wx_fmt=png&from=appmsg)

实际操作be like（以下展示均为加速画面）：

![](https://mmbiz.qpic.cn/mmbiz_gif/YicUhk5aAGtAqFvhUzJ7gBb1xsEW1gXDm2bPJw3IQaHp5mEhROAO9YozI17qGZoetpk7j6OAEdXz6RnYgpOoQ5Q/640?wx_fmt=gif&from=appmsg)

在“将不同形状杂物收纳至筐中”的任务里，WALL-OSS同样表现稳定：

![](https://mmbiz.qpic.cn/mmbiz_gif/YicUhk5aAGtAqFvhUzJ7gBb1xsEW1gXDm6iazqCJw3SEUahSPrqLwNRO9dGjDheFHWpdr1s6eXZO45WWs91iaH6WQ/640?wx_fmt=gif&from=appmsg)

该任务中，无论是得分还是成功率，WALL-OSS都高于pi0。

![](https://mmbiz.qpic.cn/mmbiz_png/YicUhk5aAGtAqFvhUzJ7gBb1xsEW1gXDmAjuvytxew6vgmWJndybtQr9mUdeoByQMv1H2PS4Lm7ZXn4enCL0DFg/640?wx_fmt=png&from=appmsg)

在“拉开抽屉并放入杂物”等需要连续规划与空间判断的任务中，也能看到其完整完成操作流程：

![](https://mmbiz.qpic.cn/mmbiz_gif/YicUhk5aAGtAqFvhUzJ7gBb1xsEW1gXDmiap9ef9dTJibicI2cy3CDkv2XqpicUUCmfZ4PYzUsNgrE5MsvefxExmNlA/640?wx_fmt=gif&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/YicUhk5aAGtAqFvhUzJ7gBb1xsEW1gXDm49ZgOG0elHmIMnialThuugsQuLzfqrYDIcMVY3EoT2QAnhkFN7eYQpw/640?wx_fmt=png&from=appmsg)

从视觉语言模型（VLM）走向视觉语言动作模型（VLA），并不是一次简单的能力叠加。

在这一迁移过程中，行业普遍面临两大核心挑战：

其一是灾难性遗忘。VLM在向动作生成扩展时，往往会牺牲原有的语言理解与视觉推理能力，导致模型“会动了，却不再真正理解任务”。
...
不同于传统多模态模型常见的“模块拼接”方案，WALL-OSS采用了共享注意力+专家分流（FFN）的架构设计。语言、视觉与动作信息被嵌入到同一表示空间中，通过共享注意力机制实现深度跨模态交互；同时，再借助专家FFN对不同任务需求进行高效分流计算。

最终，模型得以在统一框架下同时承担理解、规划与动作生成任务，形成紧耦合的认知—行动闭环。

![](https://mmbiz.qpic.cn/mmbiz_png/YicUhk5aAGtAqFvhUzJ7gBb1xsEW1gXDm8iapmKCOhW5fN81EzfibInMB02D9sq7oEbkoexiaFibvfUYqvhNzbCostw/640?wx_fmt=png&from=appmsg)

在训练策略上，WALL-OSS设计了“启发阶段（Inspiration）→整合阶段（Integration）”的阶段式范式。
...
![](https://mmbiz.qpic.cn/mmbiz_png/YicUhk5aAGtAqFvhUzJ7gBb1xsEW1gXDmydXRZf2bFtMyK1s7gCBGRtqiaYbNhdrwnDfhX8erUhsQYRWKrLCVcOQ/640?wx_fmt=png&from=appmsg)
...
WALL-OSS构建了一套统一的跨层级思维链框架：从指令理解，到中间推理，再到子任务拆解与规划，最终映射为连续的物理动作执行。
...
![](https://mmbiz.qpic.cn/mmbiz_png/YicUhk5aAGtAqFvhUzJ7gBb1xsEW1gXDmy5LxZDRbibiadkTY2q6j36aYaicu6WMQhdr2hib8WUiavl5s3jb2ry6yD4A/640?wx_fmt=png&from=appmsg)
...
目前团队已完成多轮融资。几个月前，刚宣布了近10亿元A+轮融资，阿里云、国科投资领投，国开金融、红杉、渶策、美团、联想之星、君联资本均有参与。

相比单一场景或垂直应用，自变量团队更关注如何构建一个可以被反复验证、持续演化的“机器人通用大脑”。

也正因为如此，WALL-OSS从一开始就被定位为面向真实物理世界、端到端统一的基座模型，而不是为某个Demo、某个任务定制优化的解法。

如果仅从榜单成绩来看，WALL-OSS已经足够亮眼。但真正值得被反复讨论的，并不是名次本身，而是它选择以开源的方式，参与真实物理世界的能力验证。
