...
publish_time: "2026-07-13 05:54:04"
...
see_count: 841
comment_count: 562
stats_updated_at: "2026-07-16T06:20:51.326Z"
is_unavailable: 0
...
![](https://mmbiz.qpic.cn/mmbiz_png/2jjfQoZLoqVrnGKib4DEaS96oxBXJf8sxF4ADn75JBrgp0PmcBLgoENyzOUdlwE0icplzdImKv4ia0oSV2H0iaVBI6Mam4BoLYBSwiaxic9TbtUbM/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/2jjfQoZLoqWInAyGnsunR1T5KtSsb37oWrFBbFETnC6oP6UR3gA7LRs7Rks3S6O31ibXuyuPYjDNqMgMLSTuZ7XAqJG4JE85POJIzhuZXIlk/640?wx_fmt=png&from=appmsg)

而且不只是代码。

在我的亲手验证中，他居然，直接跨了我的项目库，还把我电脑上Claude Code的配置文件一起传了上去，甚至，包括了我的某个API密钥。。。

![](https://mmbiz.qpic.cn/mmbiz_png/2jjfQoZLoqWS6xu3BSLVkibIKnA0J3dcj9ibPWLYkcKgiaC2umCjnCfg7ia2372tJiazTcNYCJdCwxMqyvlomaCiaJnLRIrG1nz5HlTEIJvL7EAcM/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/2jjfQoZLoqXoUmNuzwvLU6NRLukb5Y7deNGe0icuMWAozPEumfneRZTeRmMvXg90GW7h8MuCo46eIzvvNnQxtW1YbrpZpr0o7kzv4IrWI62o/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/2jjfQoZLoqVaNAJUorH6u5z7qqXUXsZG675KG8ib7HPEwBmiccCrUY8qQV68I9ncyTiccJkTyhianUDQ2LlOMYpBiciciaftc7TpllRUZYgng960Xs/640?wx_fmt=png&from=appmsg)

然后反混淆了二进制文件。

打开的瞬间我就知道，这事肯定是真的了。

![](https://mmbiz.qpic.cn/sz_mmbiz_png/2jjfQoZLoqVUjZ4BrYnaSyjwibsszgS6NCtsrllseOeO9o8SUMBuvYtv9o02j4XNpTKsQicUom8kNkEhcdecygnHKibhkeOaEy5trKHEXLsE8Y/640?wx_fmt=png&from=appmsg)

二进制里赫然写着repo\_state.upload、before\_codebase、after\_codebase.tar.gz、gs://grok-code-session-traces，以及上传成功、上传失败、上传禁用的完整执行分支，这尼玛是一条完整的、生产级的上传管线。
...
即使模型没有调用任何文件读取工具，Grok CLI仍然成功上传了before\_codebase.tar.gz和after\_codebase.tar.gz（也就是我的代码库的之前的状态和AI介入之后的状态），加上会话状态、对话记录、配置和日志。

全部传到了xAI的谷歌云仓库上。

![](https://mmbiz.qpic.cn/mmbiz_png/2jjfQoZLoqUFe7yparqVsqDUVRZXADCSD1XGKicASgX5j7lhaxFVG2IDfeIbPqkUKhOHMIDUjSrjJu8AhPc8Kdo7LNkGiamGlqgtkeTS6MEvA/640?wx_fmt=png&from=appmsg)

但真正让我血压飙升的，是上传包里的内容远远超出了当前仓库的范围。

它不仅传了仓库里的代码，还传了仓库之外的东西。我的~/.claude.json、我的Claude Code设置文件、我的全局AGENTS规则、我的30多个Skill文件，全部被标记为supplemental\_file（也就是补充上下文文件），一起塞进了上传包。
...
我检查了被上传的文件列表，发现我的~/.claude/settings.local.json里面有一个env.MIAODA\_API\_KEY字段。
...
![](https://mmbiz.qpic.cn/mmbiz_png/2jjfQoZLoqVqYkRyRyiaDibe1OAD8mFwpr3vmPAtibNdibm6BjAhoO2BoxKbjas7N9HQpu0FONxAibBEh7xHO6UicAZzaUa7wfTRT0ia0ialkv2IzKA/640?wx_fmt=png&from=appmsg)

这个安全研究者cereblab，应该是第一个发现这件事的，他不仅抓了包，还做了一个复现仓库，保存了完整的网络请求记录。

但关键转折是他保存的一份7月13号的xAI服务器响应。里面同时出现了两个字段，trace\_upload\_enabled等于false，以及一个新增的disable\_codebase\_upload等于true。

而他最初抓包的时候，同一个0.2.93版本的客户端，收到的配置是trace\_upload\_enabled等于true。

![](https://mmbiz.qpic.cn/sz_mmbiz_png/2jjfQoZLoqVcVKclUrq8ypyybsiatW2WrT3R0sibYIlz12m8y1Bgopctb8LuBPlpx5jHiaSwFooyQQoFVc6qvW8BM5mngPXFPWyGXiaPdibicLSAM/640?wx_fmt=png&from=appmsg)
...
7月13号凌晨，cereblab发现xAI服务端新增了disable\_codebase\_upload字段，上传被远程关闭，但是所有的配置都留下来了，后面其实也是可以无感开启。
