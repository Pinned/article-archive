...
see_count: 234
comment_count: 30
stats_updated_at: "2026-07-04T11:28:53.195Z"
is_unavailable: 0
snapshot_reason: "网络同步补齐"
topics: "[\"世界模型\",\"4D建模\",\"推理加速\"]"
entities: "[\"谷歌\"]"
---
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/UicQ7HgWiaUb1QR8LQCv1HZkXean71eNJDCXiaU1ibcCl3GwmiayR4bcqSRaeXwiaFnaiaW3shLTEZWfqVls2viczcXuDQ/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/UicQ7HgWiaUb1QR8LQCv1HZkXean71eNJDcArqyYDaujxlxsapia1ZiaokuPmibYV3dcUx7n6j3LJtfZqVEnMgaZKaw/640?wx_fmt=png&from=appmsg)

这篇论文在计算机视觉领域扔下了一枚关于「效率革命」的重磅炸弹。

它把原本割裂的 3D 重建、相机追踪、动态物体捕捉，统一成了一个极简的「查询」动作。

![](https://mmbiz.qpic.cn/sz_mmbiz_gif/UicQ7HgWiaUb1QR8LQCv1HZkXean71eNJDIDZqFkm6AP1JGibiagwY5MUa4jeChje5jb43xbtXysTkia0H4zMTmTL9Q/640?wx_fmt=gif&from=appmsg)

**更重要的是，它的速度比现有** **SOTA****技术快了 18 到 300 倍**。

![](https://mmbiz.qpic.cn/sz_mmbiz_png/UicQ7HgWiaUb1QR8LQCv1HZkXean71eNJDicQtT66GbGpDFtibc6f724t1kbEUcrTvP4MPYK17Zkk3hiblH086ku3TQ/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_gif/UicQ7HgWiaUb1QR8LQCv1HZkXean71eNJDYHdL1Qv5gXWFw27XPe9IESjYkEKkhvPgh3yjQnERTlEP40ibRSTKAibg/640?wx_fmt=gif&from=appmsg)

更令人印象深刻的是它的**全像素追踪**能力。

你可以点击视频中花瓣上的任意一个像素，D4RT 就能画出这个点在过去和未来的完整 3D 轨迹，哪怕这个点在中间几帧被蜜蜂遮挡了，或者跑到了画面之外，模型依然能根据上下文「脑补」出它的去向。

![](https://mmbiz.qpic.cn/sz_mmbiz_gif/UicQ7HgWiaUb1QR8LQCv1HZkXean71eNJDYfgibhTNz6Ej1jhRtPXzn0otn0XOYbuibdwa9gJGAxDBiaAfKWQK0ibbhw/640?wx_fmt=gif&from=appmsg)

这种视觉效果给人的感觉是：AI 不再是在一帧帧地「看」视频，而是把整段视频吞下去，在大脑里生成了一个完整的、流动的全息全景图，然后你可以随意从任何角度、任何时间去检视它。

![](https://mmbiz.qpic.cn/sz_mmbiz_png/UicQ7HgWiaUb1QR8LQCv1HZkXean71eNJDChaa54n0tQYib0PADibxqGSXSGPa0dEyKXicp9PFqS1o69OSlP6PHGhsw/640?wx_fmt=png&from=appmsg)
...
这里的「300倍」指的是**吞吐量**，具体来说是「在保持相同帧率（FPS）的前提下，模型能同时追踪多少条 3D 轨迹」。

*   **数据对比：**在 24 FPS 的标准电影帧率下，之前的强者 SpatialTrackerV2 只能同时追踪 **84**条轨迹，再多就卡了；而 D4RT 可以轻松处理 **1570**条。如果是和 DELTA 这种更慢的模型比，那就是 **314 倍**的差距。
...
在训练阶段，它依然是一个庞然大物。它的编码器使用了 ViT-g，拥有 **10 亿**参数，并且需要在 64 个 TPU 芯片上训练两天。
...
D4RT 的第一步是使用一个巨大的 Transformer 编码器（Encoder），把整段视频压缩成一个**全局场景表征（Global Scene Representation, F）**。
...
*   如果你想做轨迹追踪：就问「这个点在第 1 帧、第 2 帧……第 N 帧都在哪？」（固定 ，改变 ）。
    ...
    ![](https://mmbiz.qpic.cn/sz_mmbiz_gif/UicQ7HgWiaUb1QR8LQCv1HZkXean71eNJDbJ8b2rH25ibSLib1ibQcicp8Hwliadkd09ibsY0KmXxgUjfoqGyciaAH9y3OQ/640?wx_fmt=gif&from=appmsg)
    
*   如果你想重建点云：就问「视频里所有点在同一时刻的世界坐标在哪？」（把所有点都映射到同一个 ）。
    
    ![](https://mmbiz.qpic.cn/sz_mmbiz_gif/UicQ7HgWiaUb1QR8LQCv1HZkXean71eNJDH2dibVQSiaOsbxib0KicefzvaiaLzibFsdQtHd4fnFmaicu5n2c2ibcNmnKkcQ/640?wx_fmt=gif&from=appmsg)
...
于是，他们在查询时顺便把那个像素点周围 **9x9**的小方块图像（RGB Patch）也喂给了模型。

![](https://mmbiz.qpic.cn/sz_mmbiz_png/UicQ7HgWiaUb1QR8LQCv1HZkXean71eNJDfLtIPcqicE9uErpeydooztQZ5cgPuSnZhmmwiaKTJvhh8HcNSUyVnLcg/640?wx_fmt=png&from=appmsg)

这就像是你让人在人群中找人，光给个坐标不行，还得给他一张那个人脸部的特写照片。

消融实验证明，这个小小的设计极大地提升了重建的锐度和细节。

![](https://mmbiz.qpic.cn/sz_mmbiz_png/UicQ7HgWiaUb1QR8LQCv1HZkXean71eNJD8TSYmiauVN3thXCWcEcqrbBFVOTBftFD4zic4VaY3T1RTPe8ePwE2tCQ/640?wx_fmt=png&from=appmsg)
...
D4RT 提供的**实时、密集、动态**的 4D 感知，正是机器人急需的技能。
...
**在** **AI****的眼中，过去并没有消逝，未来也不再不可捉摸，它们只是同一个四维坐标系里，等待被查询的两个不同参数而已。**
...
https://storage.googleapis.com/d4rt\_assets/D4RT\_paper.pdf
