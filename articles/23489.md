...
see_count: 37
comment_count: 0
stats_updated_at: "2026-07-09T02:59:36.749Z"
is_unavailable: 0
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqE3Ul22aQ1ibN2WneGqI8HHhddqtibBCVUXk1icEoDGJIWsu6YHZVmNnvRhibsnNS8VWpFgh1COw4sBOwggzJPbqkvz24Fbmq1FTm4/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqG9kAHfPpCzNR5bDF9Vpg1utEMuwaMcERk1LU2FXXdCmqsfiacJbuPmn3nLH6wqCbNbiaqibasUxRQ2NIWzxkunfavNmRObwTmTQk/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_jpg/5L8bhP5dIqFkTe50nm6v8rEAfGVZN42uXUAemlL0bFsibXygYbn8fyZ6v3ibDic2VTp5Wkql2F1aGdrf9mhem4DUan59fPeTiaTeJdSK3OSvicZw/640?wx_fmt=jpeg)

  

这里的![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqFAd0rtPI7mBdKGSrDVkty6TtsnAncej6eIib2gypib7S4ic8Cr6CL915NdygE0TlZIvjbPTfD0G7NfWIRkGibicQIeITWxiajia0XRSM/640?wx_fmt=png&from=appmsg)可以是内部工具也可以是外部工具。在这样的视角下，目前大部分 Agent 基于的 ReAct 框架其实可以看做是一个特例，即将内部的模块看成是整体的 reasoning，那就变成了：

  

![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqFMOkEXlqygzoxHNPAibb5DoMPSpzUf8bIPcumHFHR08zzSRZzFdhm0wa68EqmS3gVal4JooFGX6GgqwwtajyftiaghUeUxEMd0o/640?wx_fmt=png&from=appmsg)

  

这里的![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqFYA9AbcQXuy4ICTxsaMH8c1GGl8pIVUYwMD1148sDEbNSoMEmVwLicEpicXB4P0RibjkU3fnC5jXia8by38SjK7r8sapZZUGs5tm0/640?wx_fmt=png&from=appmsg)。图 2 展示了具体的区别。这个转换看起来只是换了说法，但它改变了后续所有问题的问法。不再是「我该先想还是先动？」，而变成了:

  

![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqGqQQ91hjaOQU905wu2fUPvCemdYAuXMJ9kAibEzrtW3O63FjrMvVBBppS3N48mBzOic2Y3jlZ6B5bJ21TDEQQxA2bibZFBZmplhI/640?wx_fmt=png&from=appmsg)
...
*   内部任务集![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqHhBDJOmq1WDiayYIYmpbucnDPJjtCFB9g2H4icA44touG8Do7JcuFRWH1CFgmJRb6ibGoibEPJARBSSxGJUV0Sz5DZCPwn35TaDMk/640?wx_fmt=png&from=appmsg)：Agent m在环境 W 中基于给定的上下文仅靠内部推理就能可靠解决的任务；
    
*   世界任务集![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqEHqo465LhFlpl8xYXfhwFGBg08KlDBoBPSVibEAIp4fu2dvAEZ4eriaF9l5HAYlEFicibKLBEgDphWVSTybO9sic8pjyIqUb5EXibic8/640?wx_fmt=png&from=appmsg)：整个环境给定的所有能够解决的所有任务；
    
*   两者之差：![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqFERtia8uF1sXxUZyRrmpUqR0gd8MJZ38nicdDtMibtPXHaH5njoCEBWQcQ811CsvxN1cSgHKqm9Uz7nVC7QJZ9PqQj8kBTKo6OHs/640?wx_fmt=png&from=appmsg)是「外部工具真正必需 」的部分 —— 再怎么 prompt、再长的 CoT 也替代不了。
    

  

![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqGoicuQBLKicZjVmo54ZCmia523HXnxicUAXQErg7PsPcibSIEyVbNlIW0zpsHzictRdQicqZVFOSXZLFibbibJp0GMdhrqnWIy3pnl2nyQ/640?wx_fmt=png&from=appmsg)

图 3. 左：一个 Agent 的「知识边界」把它能内部解决的任务和剩下的世界任务隔开。中：多个 Agent 都能内部解决的部分 ——「最小任务集」。右：这群 Agent 中任意一个能内部解决的并集 ——「最大任务集」。

  

关键点是这条边界「和模型强相关」。弱模型的![](https://mmbiz.qpic.cn/sz_mmbiz_jpg/5L8bhP5dIqHZ1XjIhM5sZ7LZKKHz4Hmdod8jemPzcEfD6fFII1W6Ng196J4jTY6Rv9bWsIgyTZiaALKP3ZgIMmu9rqdxibzpPtxJzeCeqIm0k/640?wx_fmt=jpeg)小，强模型的大。所以不存在一个放之四海皆准的「工具使用准则」——GPT-4 适合的 RAG 频率，绝不等于 Llama-3-8B 适合的 RAG 频率。

  

如果要用一个画面来总结前沿 Agent 研究的主线，就是这条边界在移动。当人们说「模型能做的事越来越多 Agent 越来越自主」—— 换成精确的说法，就是 ![](https://mmbiz.qpic.cn/sz_mmbiz_jpg/5L8bhP5dIqHZ1XjIhM5sZ7LZKKHz4Hmdod8jemPzcEfD6fFII1W6Ng196J4jTY6Rv9bWsIgyTZiaALKP3ZgIMmu9rqdxibzpPtxJzeCeqIm0k/640?wx_fmt=jpeg)在变大。
...
如果继续沿着「推理和行动只是 effort 分配」这个视角往下推，一个更有意思的结论会自然出现：无论 Agent 选择内部推理还是外部调用，一个任务真正需要消耗的「认知负担」其实并不会消失，它只是在不同位置之间重新分配。团队引入了一个量，叫认知努力(epistemic effort)![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqHWRQQv4AEM6x8CJLVqiap1S4memTFHRuoG4EwRb8gUss7uIIooEObA7kv1Gczg2YU9PDSb9DDFFA3kF0ENT2SNWH13ZMmVwdkY/640?wx_fmt=png&from=appmsg)：一个任务在被解决之前必须付出的最小信息负担。任何能解决这个任务的策略![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqECvF3yqtFZ01pGBibvbYBGnhntgvW6Hw8RDrxV2G2P11yM1hiaZ93nDH3iaTelA2JEtA6vmib4gOeLxF2ZL8OHy7WicbibZULEcwZG0/640?wx_fmt=png&from=appmsg), 都必须付这笔账 ——只是付在哪边的问题:

  

![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqHQRI6yYicXLkuNmKJLSqFc4FEGsDe9btibWMHxgUcAnC3MR3Fw7Z3kbDnHDYRsYnkWhWzadgsHRicMwOvZiauQHP3F5a1Sbc5nHLg/640?wx_fmt=png&from=appmsg)
...
不管走哪条路，做一顿饭的总成本——「这道菜该长什么样」的信息量 —— 是固定的。工具只是把这个成本，从「我们的双手」搬到了「商家的厨房」。这就是![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqHWRQQv4AEM6x8CJLVqiap1S4memTFHRuoG4EwRb8gUss7uIIooEObA7kv1Gczg2YU9PDSb9DDFFA3kF0ENT2SNWH13ZMmVwdkY/640?wx_fmt=png&from=appmsg)的守恒。这个结论一旦被认真对待，当下整个 Agent 研究的风景线就被重新解释了一遍。
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqHje2ojaD2iaCY5gicAoadnDbkLml4my2erRjDkPPw9KIo1RPHDWI4dRfODT3QTnncq4E8zhy1kWjIibEFSmN0LeoCsx1Y459NrYg/640?wx_fmt=png&from=appmsg)

图 4. 横轴是内部推理投入，纵轴是外部行动投入。斜线是最小努力前沿 βE\_{int}+E\_{ext}=E^\*。左：任务在内部可解，两种努力可以自由互换；右：任务超出内部能力，外部努力存在一个不可削减的底线。\*
...
*   点 C：纯内部推理，零外部调用。任务在![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqEHXUBUwibQXbwgKBUnib3ucJLhz2NjOmBJ7Odq0dPicEyBJWwLxoeicSUBPWtWurVKNqicznbf9UFY8xo0Hkxr15ib86MdH1YLkPdM8/640?wx_fmt=png&from=appmsg)时，这是最干净的做法。
    
*   点 B：内部推理已经「用尽」, 只付最少的外部代价。任务在![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqErGStjqkJ9iawicgQZfarhl24PfR1RXVpGKZPrT7aezRO4WJxAEjKsb0MsqNB15psHx0GV7SCIWQ7HAgQ7O0ujd6gN7ErnaItyw/640?wx_fmt=png&from=appmsg)时，这是最理想的前沿。
...
按定义，panel (a) 中线段 AC 上的每一个点、panel (b) 中线段 AB 上的每一个点，都是「理想分配」。它们全都卡在最小努力前沿![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqFlFrnsUkCYTMjKASUYyHHlghxK7lhQnPsgtaBQiaTXalM1AhoVmhiaianljYQhI3RTAOTo6fFT4pYHpWfJdDoUv0Z059pwJHl2b0/640?wx_fmt=png&from=appmsg)上，全都以零浪费解决任务。
...
这里有一个需要注意的点，前沿方程里有一个 β ，它不是一个凑出来的数学符号，而是一个非常具体的现实参数![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqE98y9Mu535FF0IywYkia7xzGBNxgK5LK5yjeeo81QT1m8vib8K008VetuRb2oNWv2YzP2fRkR9hRq9xuWRengib1QdtI0xpqCOrU/640?wx_fmt=png&from=appmsg)，即「一单位内部推理」相对于「一单位外部调用」的代价比。换成大白话就是在我们开发的智能体系统里，想一会儿有多贵，对外调一次有多贵？

  

*   ![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqFWSZt2IJyD6aVibhXOZoPIJ9YyYPFZaAc2K50crRYu6DxUxgibibHicqO4MbOqSrY1KylCrLiabvasAcW5qzMuE4EHFTSPahjiciapoM/640?wx_fmt=png&from=appmsg)大：内部推理本身很贵。比如用一个参数量很小的模型做长链思考，每多推一步都在烧 token; 或者延迟敏感的交互场景，多想两秒用户就不耐烦。
    
*   ![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqEickNibyvsYbBkQ2sGmicpew80A78DLj3pZRiaCT3tPZCxIjvzhsDw0CwDYRLLdrypzIIwqicsH4BvCdg1r8TTp7Bcwhh8lbFZl1UY/640?wx_fmt=png&from=appmsg) 大：外部调用很贵。比如每次 API 要花钱、要写回数据库、要触发真实世界里不可逆的操作 (机器人抬胳膊、下单付款、调用人类专家), 或者涉及安全 / 合规审查。
...
两条路：(1) 按能力定制数据集—— 每个模型有自己的 Q\_{int}, 这个干净但贵；(2)选择性求助—— 训练 Agent 只在低 solvability 的上下文下主动外求，近似一个保守的能力上包 Q\_{max}, 更通用但精度妥协。
...
1.  怎么测量 Q\_int (m,W)？内部任务集是潜变量，只能估计。self-consistency、draft confidence、hidden-state probe、基于 world-model 的 solvability estimator 都是部分答案。一个好的内部可解度代理，会立刻成为 alignment 训练的核心零件。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqHvyWkqApJibWtP4FmYWWzDE60dSRwCKXT0Eb1mU4860rticicBAibTKtcs66Vicuq1K1Bc4CMkKGaaU53lpzAHfO1nhlSlyQ64iaXGg/640?wx_fmt=png&from=appmsg)
...
*   静态世界：这是一个「覆盖」问题。当![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqFkEIGo947JGMbSO68w0LbLlkibjyAfdaC0XH6f9gJbCrMMO6u2hhOAkVdicgE2f12tNSia2mKjzYn85aWMP1X4Xhbou1hiafhLmUk/640?wx_fmt=png&from=appmsg)是固定的时候，自进化就是一个单纯的覆盖问题 —— 知识边界不断向外漂移，把那些原本必须外包给工具的任务，一点点「收编」回自己能直接解决的范围里。它的渐近目标是: 
    
      
    
    ![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqGJNBwZib8sQzibib5UgzzaurpLrIibaMydMtTXBpn32bTialmx2mricCdWnH4iblgz50WKTib8uZ42vkyVrsZZArIGsqB72YBicRUqRicsU/640?wx_fmt=png&from=appmsg)。 
...
*   动态世界：这是一个「速率」问题。但真实世界从来不是静态的。W\_t 会随着时间不断冒出新任务 —— 新工具、新接口、新领域、新问题层出不穷。这时候，自进化就不再是「能不能追上」, 而是能不能追得够快。它变成了一个严格的速率条件:
    
      
    

![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqEeWvIh1qWTOb3Kc8tfJic7fC5d0iaFjfwfUp1UxibnckeN7QjdCNviasq5EL6l65tyXhxORgQfM8T2tW6YEZibAUlYQxg8n9dVhbo8/640?wx_fmt=png&from=appmsg)

  

一旦这个不等式不成立，知识边界相对于世界就是在停滞 —— 哪怕![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqHQxwwsTddes43ShrgPczD76oxe6dpkhn6A8c3bANlYTBEZaTT7Jbk7ZtRnWa9KXvTxicPKGiaFnTj2orqFHPXkrc1amHGGcusR4/640?wx_fmt=png&from=appmsg)自身在孤立地看时一直在变强，外部委托依然会是结构性必需，永远甩不掉。
