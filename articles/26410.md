...
see_count: 152
comment_count: 4
stats_updated_at: "2026-07-09T02:57:59.664Z"
is_unavailable: 0
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqGwWgCAI3wzMw3piaLic08xb2A1H226AHCUzIvTF2SnfCnICicc79EQZlAtE8Bw6eUet1Niatn1vdYd4DEdaYX64NNgO7bO42ycXkI/640?wx_fmt=png&from=appmsg)

  

*   官网链接：https://microsoft.github.io/SkillOpt/#idea
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqFAJxZSDDqprlbUwuVic7H9zn2jc8jXOsG72RMbuNGICFqictYVLrye9tq7JuJd7lWU7fk3R8OYTUSM6dyxIMKsV2Lts8HeR8xgc/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqGFmz9F76ZjuKo3JXI5NUQjbY5csxUDX00OiceoQVos7eG54WqicuF9S5zKA9tIiaPh3P1NslACP4VrfCicQkD0SITsoRdOlF85Yfs/640?wx_fmt=png&from=appmsg)
...
*   Rollout（前向传播）：冻结的目标模型拿着当前版本的技能文档去执行一批任务，记录完整的执行轨迹，包括消息、工具调用、验证反馈、最终得分。这一步产出的是「证据」，相当于神经网络的前向传播结果。
    
*   Reflect（反向传播）：一个独立的优化器模型分析这批执行轨迹。关键设计是，失败案例和成功案例被分开反思。失败的 minibatch 用来发现「哪些操作规则需要修正」，成功的 minibatch 用来确认「哪些现有规则在起作用，不能动」。这一步相当于计算「文本空间的梯度」，告诉系统技能文档该往哪个方向改。
    
*   Edit（参数更新）：优化器模型基于反思结果，提出对技能文档的结构化编辑操作：添加新规则（add）、删除失效规则（delete）、替换需要修正的规则（replace）。
    
*   Gate（验证门控）：候选的新技能文档必须在一个 held-out 的验证集上跑一遍，只有性能严格提升时才被接受。这一步防止过拟合，确保每次更新都是真正的改进。
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqH0kfeWvszCT8bf50Rzx2CfnibljxUGDibQnYC6SxERiaibt46cvcg1EFdC0WqjYDbxkStRAdOSXAib7CXE8tT8thcIbjXSbMYicma8M/640?wx_fmt=png&from=appmsg)
...
关键的是，这两个机制只在训练时存在。部署时，目标模型只需要那份最终的 best\_skill.md，不需要任何额外的模型调用或记忆模块。推理时的开销为零。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqEs84p26So6bQaGDd2iabm4XVNDS8vN3DhrdqF5AvZaauzHBFKwoDt4tbdM2UQzEfiap3F7k0zYfia0f4C4CljrLCMfDBES7Lt5kI/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqGz7SBpVplWZL1XTT2ibkKtGxgaoP6oRT6cXPZ5iap30zt8ZiaSGpu8aSrUwmXtVKKq8CD1cRjYoEhUfSL3VRiby3QtH9R1I1nZR50/640?wx_fmt=png&from=appmsg)
...
*   部署极简：最终部署时只需要一个 best\_skill.md 文件。不需要优化器模型，不需要记忆模块，不需要任何额外的推理开销。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqH9TRe2wQFh1CxjLOTSKafpb9iaNKUfNRRvCGTLLsHSpm0mqI87V3a62lxx194fNWzficzuSyVqdBSgjuMRu8dKJ8Yic92icP98HWk/640?wx_fmt=png&from=appmsg)
