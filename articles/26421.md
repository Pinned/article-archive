...
url: "https://mp.weixin.qq.com/s/3wDmtR64sjA2ARCElvoBnQ"
...
see_count: 23
comment_count: 0
stats_updated_at: "2026-07-09T02:58:13.021Z"
is_unavailable: 0
snapshot_reason: "网络同步补齐"
topics: "[\"注意力匹配\",\"显存优化\",\"大模型\"]"
entities: "[\"MIT\"]"
---
...
![](https://mmbiz.qpic.cn/sz_mmbiz_gif/5L8bhP5dIqE3YDwDRg411pfVBs6dRicHkTpAsZq9INFu7b2sIR5ACgu0TYCOB4IsmeVsibZagpYeCP35wuavkQrAmzHibBnlpd4Xbtnpev4Ro0/640?wx_fmt=gif&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqGlVy3iciaFlgEibibPAWhxe01bBGINYRSjjccUgDFDQMvz63kAxeA7iaib9vIvCZ1ib0VJtibSt8L9TGQInxVTX7pzVkmAENORJwR0Xok/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqFMTL5ada9mXAmyia3PnCBibEdnrcJ5cibYBFXdUnaGYQ9icV6cY5G1lC3fsW56lX5EWyx819OQ7VevYe5D6ZpNGbGPDNbicmJYJmzg/640?wx_fmt=png&from=appmsg)
...
为了完美欺骗 AI，让它觉得「压缩后的记忆和原本庞大的记忆一模一样」，压缩后的键值对 (C\_k, C\_v) 必须严格匹配原始记忆的两个核心数学属性：
...
如果用严谨的数学语言（如论文中的公式 1 和 2）来表达，他们要优化的目标就是找到 (C\_k,β, C\_v)，使得对于所有相关的查询 q：

  

![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqHy6kQ4xZ4ky2Sib4ibtEv2aOFHnq1RKcmN4uDViaGicl5eVujZQNnfsxaB0vJAwRxotVppZbCPMPqs16twSEt7IAnVlyjoggNJwjI/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqETmsjQpx2xA6ycJFJm7zL3hpmOhcNtF7VjibSowHia9YibeZdBc9US8R8wib6FvI8LUhzH8YTXDibBJu0cQ2YkZvTIt7xlfLxepU9Q/640?wx_fmt=png&from=appmsg)
...
*   首先，锁定 C\_k 后，质量匹配问题退化成了一个非负最小二乘法（NNLS）问题，瞬间就能计算出偏差 β。
    
*   随后，注意力输出匹配问题直接变成了一个标准的普通最小二乘法（OLS）问题，通过简单的代数矩阵运算，眨眼间就能求出压缩后的值 C\_v！
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqFopvsC1MxcLIEZzQyIYicXB68KT6kNibic07ghnTVHXPJON9ibOSGf3froNib6nFXVbn36ibibyzhTTvShDdm3OrWrh2RkDDTS3icZkdo/640?wx_fmt=png&from=appmsg)
...
有了数学武器，接下来的工程落地同样惊艳。为了让压缩算法知道该保留什么，系统需要一批「参考查询」（Q\_ref），作为模型未来可能提出的问题的「替身」。
...
*   重复预填充：悄悄在文档末尾加一句隐藏指令：「重复前面的上下文」，然后捕获模型在试图复述时产生的内部 Query 向量。
    
*   自我学习：让模型对文档进行快速的合成任务，比如「提取所有核心事实」或「把日期结构化为 JSON」，从而嗅探出模型在深度推理时会生成什么样的 Query。
    

  

手里攥着这些极具代表性的 Query 探针，系统开始从原始的茫茫 Key 海中挑选「金钥匙」（C\_k）。论文中提供了两种方法：

  

1.  最高注意力法（Highest Attention Keys）：这是一种闪电般的启发式方法，直接挑出在参考查询中被关注度最高的 Keys。速度极快，性价比超高。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqGiaqCkoSNgQ74n4fWaMloS4gjJUTXBCfTrJvFpiazASpxwdYTswibqOpjJfvQn6LoNyyiaAbQVwJgYAxnw54vf4A2wvP6aVMKumu0/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqHND4XooJYPKjN6cD3P2AyaQq742ia0iaTvNzsft1p5xoYHp1OsJ37fAtpys5bz72iatqqCrPdCEjoP2o8K4T3OmIpg2opn4nibS5w/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqHLOblfaiaAakRFRzk80NkcvLswlYYGIFPccLiauN0HibHRtYwhYpaDibiceKiayOcRuFNtiaE9PCo86oq39vzqmSaY1yzPtDI2EqltB8/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqHBU7RXhmA5LwLBIfC0L5wK3TfxaMDyOlFLf34qxqax4KUoOgiaDfepiaHmgm2Ribs4RsYf0wOF6JtE46DHL41JjicQibrSHvicVD1eI/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqE84oFrOlk1EtOaSUlBrJNLxN2uNgGRjzfxdVn1GRia95H2PZzcrTVA4RTic22bNhETSpLL6kRZoItJg2AR2XFOA0AGa2lIlQ11s/640?wx_fmt=png&from=appmsg)
