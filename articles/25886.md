---
id: 25886
title: "存读档延迟首次压到毫秒级！上交华为推出智能体沙箱系统"
url: "https://mp.weixin.qq.com/s/rMQLogtiDSJp7hlOpO9pXw"
...
see_count: 34
comment_count: 0
stats_updated_at: "2026-07-08T09:59:21.646Z"
is_unavailable: 0
...
![](https://mmbiz.qpic.cn/mmbiz_png/A6fTew8FFGEe2dW54ImGUQcCgG4eQKUoibKSOQF8FftmohsZaXmX94ZJicoDycENhX1I78Oag70ae90HgtFXWKicOFHDHMZve04ZhTVeuU6kks/640?wx_fmt=png&from=appmsg)
...
为了打破这个紧箍咒，**上海交通大学并行与分布式系统研究所（IPADS）与鲲鹏团队**联合出手了。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_jpg/A6fTew8FFGFxKsiaIaAI6yD16IiaFCgk4EBa9icDwYQDjoPVG8KHT8sdnMLNDQMNgibfkTdpicXp04VYs9S4CRFPQz4XMeoLkuk2qjlLzcnzVMec/640?wx_fmt=jpeg)
...
在早期的纯文本Agent任务中_（比如做选择题、快问快答）_，回滚几乎是零成本的：只需要把Prompt历史里后面那段删掉，假装没发生过就行。
...
这时候，Agent要想后悔，就必须实现**文件系统**和**进程内存**的原子级同步回滚。
...
存读档太慢，直接导致了今天所有的主流Agent框架_（如OpenHands、Aider、SWE-Agent）_都在做妥协。
...
![](https://mmbiz.qpic.cn/mmbiz_png/A6fTew8FFGHQamFjLBfpE3HWKRYoQuCGZV5xsCqibic7C1icFhxATxqcqLeib7HKFbH3ECX6A1ibFuLXtno3GXibsdygJY4WicSAqKjh8JhsLVGvtc/640?wx_fmt=png&from=appmsg)

DeltaBox的毫秒级神话的存读档能力，是由两个在内核和用户态高度协同的黑科技来实现的：

#### DeltaFS：智能体文件的增量存读档管理

![](https://mmbiz.qpic.cn/sz_mmbiz_png/A6fTew8FFGEoia8I6UrEhlfpnoYdyZXkUOZVNG9c0zCDf4WhzkqsYKk9KjrDghe055aNpI0E8zylDjUhtua9R8DZq9R7ttn0NPPK2WWWEMFI/640?wx_fmt=png&from=appmsg)
...
DeltaBox团队直接在操作系统内核动了刀子，基于Linux OverlayFS，构建了全新的DeltaFS智能体文件系统，引入了**运行时热层切换（Runtime hot layer switching）**的能力。

现在，Agent每前进一步：

*   DeltaFS**在不卸载的前提下，瞬间将当前的可写层冻结为只读层**，并在其上无缝插入一个新的空白可写层。
    
*   配合底层的XFS reflink技术，文件复制优化成了极轻量的“引用指向”，物理块只在真正发生改写时_（4KB粒度）_才会发生写时复制。
    
*   进程此前打开的文件描述符，通过设计独特的**延迟更新机制**，按需透明地重定向到新层，整个过程应用完全无感。
...
![](https://mmbiz.qpic.cn/mmbiz_png/A6fTew8FFGHHXXckSPMCTJT9QdWY298DUZrSjcRTJMYOWnKnlZibcwEIf3FKOAwe0QwA3KDKzyKz86VTgicW58H7zzIO4S2joFiaACBk3srAWg/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/A6fTew8FFGG8ibc6ywANic1GbTMGsXlXrBoqCEgdjFDcQicCbFqr9BXkINZVyErh0fAYk63d7lzw6UpLpoRKQTk1VIMGJyW3tLevXfica5vzbto/640?wx_fmt=png&from=appmsg)

*   存盘（Checkpoint）：Agent感知的阻塞开销为0毫秒_（实际耗时10.83毫秒被LLM等待时间完全吸收）_。
    
*   读档（Restore）：快路径_（模板命中）_仅需1.86毫秒，即使模板淘汰不幸触发了慢路径_（优化后的CRIU恢复）_，也仅需9.29毫秒。
...
![](https://mmbiz.qpic.cn/mmbiz_png/A6fTew8FFGG1814g2Vz03PPIbYHuCicpmTlcLe3IVsPO6pER3fsxAGh2a1hKYTg6tCxq857q9vQ8HXIx60E1FnjcNfqUA8zecyJCh0WHLsVQ/640?wx_fmt=png&from=appmsg)
