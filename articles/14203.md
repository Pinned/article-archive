...
url: "https://mp.weixin.qq.com/s/ZbmPC3DJDkokZ8J9eZH_0g"
publish_time: "2026-04-27 13:22:58"
account_name: "InfoQ"
read_count: 17412
like_count: 177
share_count: 871
see_count: 77
comment_count: 20
stats_updated_at: "2026-07-09T03:13:36.615Z"
is_unavailable: 0
snapshot_reason: "网络同步补齐"
topics: "[\"开源经验\",\"功能堆砌\",\"开发反思\"]"
...
![](https://mmbiz.qpic.cn/mmbiz_gif/hLLZnAbUwNg3ZBUT1XCdPQ3uUYicMj4NbQkkJEBB3CSu9UiadfvfdSBok2KvUer05Ihwgbmu2ic9sMKT6J4ozyKINO7IA3o2fLMenZtAQsicNpE/640?wx_fmt=gif&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/hLLZnAbUwNgiaLWZBbn6RBb2ARBO4qfq2MES1Am3XrXhsry123afkOFlwlguJiamy0YNRKC6Ij7riavazczFmOUGFHJKknUqUtHoOJKtEHd2hM/640?wx_fmt=png&from=appmsg)

后来写 Ghostty 的 Mitchell 也忍不住了，他说：“这听起来有点冒犯。别把锅甩给 Ghostty 或者其他终端，纯粹是因为你的代码太烂了。”终端渲染一帧只需要不到 1 毫秒，每秒能跑几百帧，所以别拿这个当借口。

![](https://mmbiz.qpic.cn/sz_mmbiz_png/hLLZnAbUwNgxxShZ6zVxAP3giaDhZUtWwVMeGKLGE80NtgMias66XJWf1Sf98icx5BonCsHteNPbibrxibicgSWA24utJ1ch6KjSKPSHHTmjqoIys/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/hLLZnAbUwNhPsicmY7ABibibicicbJUcuxjNY4pdNAjeggvgEWWBd1qo0latlNoQjtWRtn5YqpT88lLdokickniaLbv5rfib7g2HsarkQ2Dr8URicdfk/640?wx_fmt=png&from=appmsg)整个系统只由四个 package 组成。首先是 AI package，本质上是对多种 provider 的一个轻量抽象层。因为不同 provider 使用不同的 transport protocol，这一层帮你把复杂性都抹平了。你可以在同一个 context 或 session 里非常轻松地和不同 provider 对话、随时切换。接下来是 agent core，一个通用的 agent loop，包含 tooling、定位、验证等等基础能力。然后是 TUI，大概只有 600 行代码，但出奇地好用，可能因为不是某个 clanker 写的。最后是 Coding Agent 本身，它既可以作为一个 SDK，在 headless 模式下使用，也可以作为一个完整的终端交互式 Coding Agent。

![](https://mmbiz.qpic.cn/mmbiz_png/hLLZnAbUwNhTPLBhYGiap9xAWOzDxjt9HvN5m3cvvhV3UupX8ULrT0cMXVPD8ibw5iadjhp6UJ4atSiaemm4A0oamg7TicTOIwQ29RjXGrITW3ag/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/hLLZnAbUwNhQ4HS3NkeF3RmVBO83YsIkCRfckgY1g5DRLWuk4FYRO7dyzu40sxKxJgdKDHWhZk1UHRKFm9XkdvyQBYVkFHMTYWia122QyZY8/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/hLLZnAbUwNjS6icc7VILXmxw1dCaxgGibMCH1s9mof66HibKdRdDdLx8dF2SmQ3MvXymxuouDzAOPRx64O0QFTibDK3TtpttqofyR6enHyWWXjQ/640?wx_fmt=png&from=appmsg)

社区里已经有不少有趣的 extension。比如有人用 5 分钟就在 pi 里复刻了 Claude Code ships，而且功能更多。

![](https://mmbiz.qpic.cn/sz_mmbiz_png/hLLZnAbUwNiaYbGnxUnv9eQPcyv3XE8guDBuSgicUx6ibIO6kEkZ6A3GmrVvmHQMpuiaY2n6lj1zawvhhMqHzZicd9RcGGrkH2DADRzN6WgLLFg0/640?wx_fmt=png&from=appmsg)

pi-messenger，是多个 pi agent 的聊天室，它们可以互相通信，还有自定义 UI，可以实时观察它们的行为，而且确实能跑。

![](https://mmbiz.qpic.cn/mmbiz_gif/hLLZnAbUwNiaAxbUGosyrB39Tc8IGN9UZWaCAicoxCAz70Eh6OXIrpibTSzosUykfV2zJq8aVhy9RjhHmH3vHxE3s2xDGzfzfjbibLiaEl8EQBy0/640?wx_fmt=gif&from=appmsg)

甚至还有一些更“离谱”的玩法，比如 pi-nes，你可以在 agent 运行的时候顺手打个游戏。

![](https://mmbiz.qpic.cn/mmbiz_png/hLLZnAbUwNgM3xyDOAUQJwIUAXefxE8FCDAttf86SQ7MVCEicole1MSbg2dAEN2Niag1CZ8emBH38WfbLwmibwha2vAQzfMG3MJyiciclHDeH6pU/640?wx_fmt=png&from=appmsg)

pi-annotate，可以直接打开你正在开发的网站，在前端界面上做标注，把反馈原地喂回给 agent，再让它修改代码。

![](https://mmbiz.qpic.cn/sz_mmbiz_png/hLLZnAbUwNiawicHpof7PMrG2REQzpNkInYQNuXK81HQliccqXrl9w3kRY3TcNsZ4IVhXM5LoIiaMjvCmrVwkyNyYSkBfibj1I7c5gY6cXAcPGvI/640?wx_fmt=png&from=appmsg)

还有我自己常用的 pi-files-widget，不用切到 IDE，就能快速查看刚刚被修改的文件。

关键在于，这些都不是内置功能，全都是 extension。而大多数人只需要几分钟到一个下午，就能把这些东西按自己的习惯搭出来。

![](https://mmbiz.qpic.cn/sz_mmbiz_png/hLLZnAbUwNjPxw3VmDjFF1Jick29evdnSEIGWp2ibC70SXd1icyBCx2DibETAq9tWK1oglNxriacDYIZyHsqjO1ymFBSHpTxG6ickicOUjo3loAPBM/640?wx_fmt=png&from=appmsg)
