---
id: 26256
title: "当AI Agent开始工作，安全该如何跟上？AgentDoG 1.5开源发布"
url: "https://mp.weixin.qq.com/s/SwQE5tKF5HyEsLcmOrkR5Q"
publish_time: "2026-06-16 14:44:59"
account_name: "机器之心"
read_count: 993
like_count: 12
share_count: 85
see_count: 7
comment_count: 0
stats_updated_at: "2026-07-09T02:55:06.759Z"
is_unavailable: 0
snapshot_reason: "网络同步补齐"
topics: "[\"AI Agent\",\"安全\",\"开源\"]"
entities: "[\"AgentDoG\"]"
---
...
最近，同事.skill（colleague.skill）在社区中的快速传播，让很多人开始直观地感受到：AI Agent 正在从「聊天助手」走向「工作伙伴」。
...
换句话说，Agent 正在逐渐成为一种新的工作单元：它可以协助人类完成任务，也可能在某些场景中接管一部分具体工作。
...
围绕这一问题，上海人工智能实验室发布 AgentDoG 1.5：一个面向 AI Agent 的轻量化、可扩展安全诊断与在线护栏框架。论文已上线 arXiv，代码、模型与数据均已开源。

  

![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqE0qCjjwPPdyEUpMFPiakkXNVEG40QC4sPpLrPzQ0RcztDoia4858N2z8gXJZmlwicfXQ8VNvOrJQHlPfvq3kGZQrrBRVibFwKXzn0/640?wx_fmt=png&from=appmsg)

  

![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqE2NmBGCT3DOaePnV85AV5bNbImPjibpeicic3iaYoeqTHQVr09Dl8wEzTrOEzmv3D3RazGEwaPWMuqsTzIxCGMhbfqHuuU2Gaaw0k/640?wx_fmt=png&from=appmsg)
...
AgentDoG 1.5 的核心出发点是：Agent 的安全风险往往发生在完整执行过程中，而不是只发生在 Agent 的最终回复里。一个 Agent 可能在最终回复中看起来正常，但此前已经错误调用了工具、泄露了信息、执行了危险命令，或者被外部环境中的恶意内容诱导发生目标偏移。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqHoFslgeQyZfVsc2T9f8pFYatiaHbwZv6W77jtVpQ3p8xPic8FLzfsbicK1cl41xibO3nUIvzlubs3kibcOQ7bI0yjWHrIAhkQdLKCU/640?wx_fmt=png&from=appmsg)
...
AgentDoG 1.5 采用的方式是：保持 Risk Source、Failure Mode、Real-world Harm 这三个高层维度不变，在不同执行场景下扩展和细化 leaf categories。
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqEoMiaibBKHmYOYOlfGLGFh135uR0sOD9Ox7ssaKEctEHjQ6rAjdMPxMp9BiaHQrj70yakMYMS2BSibXFjawB3ODh1YBPVziaQWjibbU/640?wx_fmt=png&from=appmsg)

  

基于这一思路，论文进一步构建了 ATBench Family。ATBench 面向通用 tool-use agent，ATBench-Claw 面向 OpenClaw 执行场景，ATBench-Codex 面向 Codex 执行场景。三者共享同一个 trajectory-level diagnosis task 和三维 taxonomy 框架，同时针对不同执行环境扩展具体风险类别。这使得 AgentDoG 1.5 能够在保持跨场景可比性的同时，持续适配新的 Agent 平台。
...
实验结果显示，AgentDoG 1.5 在轨迹级安全判断和细粒度风险诊断上均取得了强表现。
...
这些结果表明，贴近 Agent 风险结构的数据和监督信号，可以把可靠的 agent safety judgment 能力蒸馏到较小规模的模型中。

  

![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqG9CvLt218dqXXKGw41bZWb87BFd8oXpicfHtql6y5eE6V8DV4hiarcqJGRqMvj7A82UrUSb5B98WaKM7dSeia0XIjZXZmvIoWv3g/640?wx_fmt=png&from=appmsg)
...
这说明 AgentDoG 1.5 不只是一个评测模型，也可以作为数据质量控制模块参与安全训练流程。

  

![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqFXSk0YaRsnLYZkdVY93AR4yeQOqQK9x1rLVRMVJxu0BJ2pZ7Z4PGtMAdGWZrf6ibBS76SQ2N6XcBF83OV9GbVibibtosEO0j9sC0/640?wx_fmt=png&from=appmsg)
...
实验中，该环境可同时加载 10,000 个环境、维护 1,000 个活跃实例，并支持 1,000 个并发工具调用，峰值内存保持在 2.5GB 以下。这一设计显著降低了 agentic safety RL 的环境部署成本，使大规模安全训练更加可行。

  

![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqHW9uZAXptjUvnN7RjQS6hLmSCX8ZFfkfjImtkZjBSiapJLe2Y31XLyTbHrYtibWqKA90pmLQJRurPKVok55eUatCpDW5iaz8c7fI/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqHkmTQUib8icibJaBCMzxeWpSv3GGZeQ5ZKDaAEJY1kvXwbJVDuJwQ5fibvicBOY6ibHcaBeE5YtDbR2ynQWQhSMLllgvalndVrXnBDU/640?wx_fmt=png&from=appmsg)

  
AgentDoG 1.5 不仅可以支持 Agent 训练，还可以部署为 online guardrail。论文提出一种 Pre-Reply 介入机制：在 Agent 最终回复发送给用户之前，AgentDoG 1.5 会读取完整执行轨迹，包括用户输入、工具调用、工具结果、环境观察和最终草稿，并判断是否允许放行。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqFasqCibLuIPicGl3LgCGUvT1l6GTicWrABkfCibET0ym9fHEMKqmauJUEIZI5oe0q12yFvxEHia3j1JDzC43MD5kOxthpvebicTciboM/640?wx_fmt=png&from=appmsg)
...
AI Agent 的能力正在从「生成内容」走向「执行任务」。当 Agent 能够调用真实工具、访问真实文件、触达真实系统时，安全问题也随之升级：它不再只是内容审核问题，而是执行过程中的行为诊断、风险归因和在线干预问题。
...
如果说未来的 AI Agent 会越来越像一个能够行动的数字助手，那么 AgentDoG 1.5 想做的，就是让它在行动之前、行动之中、行动之后，都有一套可诊断、可扩展、可部署的安全机制。
