...
url: "https://mp.weixin.qq.com/s/kHyc7zbzb5U53KVOO6oV5Q"
...
see_count: 17
comment_count: 0
stats_updated_at: "2026-07-09T02:59:46.189Z"
is_unavailable: 0
snapshot_reason: "网络同步补齐"
topics: "[\"大模型优化\",\"推理效率\",\"测试时计算\"]"
entities: "[\"清华\",\"TaH\"]"
---
...
> 来自清华大学、无问芯穹、上海交通大学等机构的研究团队提出 Think-at-Hard（TaH）：一种面向小模型的选择性潜空间迭代方法，让 Looped Transformer 只在真正困难的 token 上多想一步，在跳过 93% 的额外迭代的同时，于 9 个数学、问答、代码基准上取得 3.0%–6.8% 的稳定提升。  
> 
...
来自清华大学、无问芯穹、上海交通大学等机构的研究团队在论文中发现，事情没这么简单：相当一部分 token 在第一次前向时已经预测正确，后续的潜空间迭代反而可能把正确的预测改错。论文将这一现象命名为 latent overthinking，也就是「潜空间过度思考」。

  

基于这一观察，作者提出 Think-at-Hard（TaH）：一种选择性潜空间迭代的 Looped Transformer。通过后训练，让标准模型变为 Looped Transformer，且只在真正困难的 token 上增加算力。本工作入选 ICLR LIT Workshop Best Paper Shortlist，并被 ICML 2026 接收。

  

![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqFv6I6nI1r0qVw3EjHEpFtkwoAvibC7MjO4RRLuxVqHH2uS3BibJYxJwUNXghForG2kU1ssjfenO4clIwics2ib84SlYGbXKN6R9zY/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqHRHrXyqlicTzK6dC5PBsKNYEptkMRViaxNTRgXqeiatNf8sPmPDqyyuKCXrFLDvfibibbUSYpJlHKvFz6C0pEbxMdw4A2GAsabVibibI/640?wx_fmt=png&from=appmsg)

  

*   论文标题：Think-at-Hard: Selective Latent Iterations to Improve Reasoning Language Models
    
*   代码链接：https://github.com/thu-nics/TaH
    
*   主页链接：https://fuvty.github.io/TaH\_project\_page
...
为了量化这种选择性迭代的潜力，作者构造了一个 oracle 策略：仅当模型第一次预测某个 token 出错时，才让它继续在潜空间迭代；如果第一次已经预测正确，就直接输出。实验显示，仅靠这个 oracle，模型在下游任务上最多就能换来 7.3% 的性能提升，而且只需要让大约 11–19% 的 token 二次迭代；如果再换上为选择性迭代优化的 TaH 架构，oracle 带来的提升甚至超过 25%。

  

这意味着推理时的计算动态分配应该细化到 token 级别：难题里也有简单 token，简单题里同样可能出现关键 token。更关键的是，对简单 token 强行多算不仅浪费算力，还会让一部分本来预测对的 token 在第二次迭代里被改错，即 latent overthinking。
...
具体来说，TaH 在模型中加入一个轻量级 iteration decider（小型 MLP）。每完成一轮潜空间迭代后，decider 会基于 backbone（模型骨干）的状态 \*，预测一个继续概率。如果低于阈值，模型直接输出下一个 token；如果高于阈值，则进入下一轮潜空间迭代。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqF2Y6ia9VRHHpibaqsYNVsAVs6hCPsByyUBeIxItX2wh8IibEolpiaYwsybAzWXF4rMpcMl4VoLEFvFjbYH5lq1fqf1ro9ODTgygia8/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqHXrdyNTib5pNQcNX4V9LSl6ibBeaIR0WARrypLJXNHbibK0L47YtNoYPNiatNcCKALmBwMicYsYTJ5TYEEGYVQj4ibKrpDHlcpp1HVQ/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/5L8bhP5dIqEicPtz9Ch7KxibNq1q7xqjLcicEEEiaFkEJHpHdibt8ibawlmsHxvrjcUVaoq5bgs19dlH3QG5ialgmWMUZsS9MOSgNefKOAMSYwWaSc/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqERzyFkDXpcy7gOXtgzAyhsJeUMIYW2SvIZseOaWbichbgDu9nD6gv1mhmGTg6500ia89wtP0iayZsg8xz3ia2eWu7GxHAbpM4Hsm0/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqGico38atiaSSr9ib7NoAibCoGIHMZCczQRTa1FxIdgibIuicOS6TG6sAUAHFlBZcVFp6IY347CoNNFkia6boyKwzHzR2IQSQDic4cGZvI/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/5L8bhP5dIqF7j2ka32brg9Mkg1m0FQlzOrUEE94oAfMyRrw0icqrpBibTAXfa5T7EMYsE0NYFcc1KWrVadGax5Aa0ibkv2O3MZrw4zqI2Nic5IU/640?wx_fmt=png&from=appmsg)
